  ...
  try {
    ...
    try {
      ...
      for await (const chunk of result.stream) {
        // Check if we've been asked to abort
        if (abortSignal?.aborted) {
          console.log('[streamGeminiCompletionWithSDK] Stream aborted by abort signal');
          break;
        }
        ...
        // Skip empty chunks
        if (!textChunk) {
          console.log('[streamGeminiCompletionWithSDK] Received empty chunk, skipping');
          continue;
        }
        ...
        // Periodically log chunk information
        if (chunkCounter % logInterval === 0 || chunkCounter === 1) {
          console.log(`[streamGeminiCompletionWithSDK] Chunk #${chunkCounter}: length=${textChunk.length}, accumulated=${totalAccumulatedText.length}`);
        }
      ...
      console.log(`[streamGeminiCompletionWithSDK] Stream complete, processed ${chunkCounter} chunks, total size: ${totalAccumulatedText.length} chars`);
