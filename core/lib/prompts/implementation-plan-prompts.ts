...
export function generateImplementationPlanUserPrompt({
  ...
  console.log(`[generateImplementationPlanUserPrompt] Generating prompt with ${relevantFiles.length} relevant files and fileContents with ${Object.keys(fileContents).length} entries`);

  // Count files that are missing from fileContents
  const missingFiles = relevantFiles.filter(filePath => !fileContents[filePath]);
  if (missingFiles.length > 0) {
    console.warn(`[generateImplementationPlanUserPrompt] ${missingFiles.length} files are missing from fileContents:`,
      missingFiles.length > 5 ? missingFiles.slice(0, 5).join(', ') + '...' : missingFiles.join(', '));
  }
  ...
  const codeContext = relevantFiles
    .map(filePath => {
      ...
      if (!fileContents[filePath]) {
        console.warn(`[generateImplementationPlanUserPrompt] File content not found for path: ${filePath}`);
        return null;
      }
...
\`\`\`
  ...
  const includedFileCount = relevantFiles.filter(filePath => fileContents[filePath]).length;

  console.log(`[generateImplementationPlanUserPrompt] Including ${includedFileCount}/${relevantFileCount} files in the prompt context`);
...
`;
  ...
  console.log(`[generateImplementationPlanUserPrompt] Generated user prompt of ${userPrompt.length} characters`);
