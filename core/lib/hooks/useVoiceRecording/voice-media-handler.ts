...
export async function setupMedia({
  ...
  try {
    ...
    if ('audioBitsPerSecond' in MediaRecorder.prototype) {
      ...
      (recorderOptions as any).audioBitsPerSecond = 128000; // 128 kbps is good for speech
    }
    ...
    if (stream.getAudioTracks().length > 0) {
      ...
      console.log(`[MediaHandler] Active audio device: ${activeDeviceLabel || 'Unknown'} (${activeDeviceId || 'No ID'})`);
    }
