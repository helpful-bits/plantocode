 ...
 */
export function contains(str: string, searchStr: string, caseInsensitive = false): boolean {
  if (caseInsensitive) {
 ...
 */
export function processText(textInput: any): string | null {
  ...
  if (typeof textInput === 'object') {
    ...
    if (textInput.isBackgroundJob && textInput.jobId) {
      return null;
    }

    // Try to extract text from common API response formats
    if (textInput.response && typeof textInput.response === 'string') {
      processedText = textInput.response;
    } else if (textInput.data && typeof textInput.data === 'string') {
      processedText = textInput.data;
    } else if (textInput.text && typeof textInput.text === 'string') {
      processedText = textInput.text;
    } else if (textInput.content && typeof textInput.content === 'string') {
      processedText = textInput.content;
    } else if (textInput.message && typeof textInput.message === 'string') {
      processedText = textInput.message;
    } else {
    ...
    try {
      processedText = String(textInput);
    } catch (e) {
      return null;
    }
  }
 ...
 */
export function isDisplayableText(text: any): boolean {
