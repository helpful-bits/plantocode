...
export function formatTimestamp(timestamp: number | string | Date | null | undefined): string {
  ...
  if (typeof timestamp === 'number' && (timestamp === 0 || !Number.isFinite(timestamp))) return 'N/A';
  ...
  try {
    ...
    if (isNaN(date.getTime())) {
      console.warn(`[date-utils] Invalid timestamp: ${timestamp}, type: ${typeof timestamp}`);
      return 'Invalid date';
    }
    ...
    console.error(`[date-utils] Error formatting timestamp: ${timestamp}`, e);
    return 'Invalid date';
  }
}
...
export function formatTimeAgo(timestamp: number | string | Date | null | undefined): string {
  ...
  try {
    ...
    // Check if the date is valid
    if (isNaN(date.getTime())) {
      console.warn(`[date-utils] Invalid timestamp for timeAgo: ${timestamp}, type: ${typeof timestamp}`);
      return 'Invalid date';
    }
    ...
    console.error(`[date-utils] Error calculating timeAgo: ${timestamp}`, e);
    return 'Invalid date';
  }
}
...
export function formatJobDuration(
  ...
  try {
    ...
    if (typeof startTime !== 'number' || !Number.isFinite(startTime) || startTime < 0) {
      console.warn(`[utils] Invalid startTime value: ${startTime}, type: ${typeof startTime}`);
      return status === 'idle' ? 'Pending' : 'N/A';
    }
    
    // Validate startTime is in milliseconds - if not, convert (a safeguard)
    let startMs = startTime;
    if (startTime < 10000000000) { // Timestamp is in seconds not milliseconds
      console.warn(`[utils] startTime appears to be in seconds, converting to ms: ${startTime}`);
    ...
    if (['running', 'preparing'].includes(status)) {
      ...
      if (durationMs < 0) {
        console.warn(`[utils] Negative job duration calculated: ${durationMs}ms, startTime: ${startTime}`);
        return 'Just started';
      }
      
      if (durationMs > 31536000000) { // > 1 year (likely timestamp handling error)
        console.warn(`[utils] Unreasonably long job duration: ${durationMs}ms, startTime: ${startTime}`);
        return 'Running';
      }
    ...
    if (endTime !== null && endTime !== undefined) {
      ...
      if (typeof endTime !== 'number' || !Number.isFinite(endTime) || endTime < 0) {
        console.warn(`[utils] Invalid endTime value: ${endTime}, type: ${typeof endTime}`);
        return 'Completed';
      }
      
      // Convert end time from seconds to ms if needed
      let endMs = endTime;
      if (endTime < 10000000000) { // Timestamp is in seconds not milliseconds
        console.warn(`[utils] endTime appears to be in seconds, converting to ms: ${endTime}`);
        endMs = endTime * 1000;
      }
      ...
      if (durationMs < 0) {
        console.warn(`[utils] Negative job duration: ${durationMs}ms, startTime: ${startTime}, endTime: ${endTime}`);
        return 'Completed';
      }
      
      if (durationMs > 31536000000) { // > 1 year (likely timestamp handling error)
        console.warn(`[utils] Unreasonably long job duration: ${durationMs}ms, startTime: ${startTime}, endTime: ${endTime}`);
        return 'Completed';
      }
    ...
    console.error('Error formatting job duration:', e);
    return 'N/A';
  }
}
...
function formatDurationMs(ms: number, isRunning: boolean = false): string {
  ...
  if (ms === null || ms === undefined || typeof ms !== 'number' || !Number.isFinite(ms)) {
    console.warn(`[utils] Invalid duration value: ${ms}, type: ${typeof ms}`);
    return 'N/A';
  }
  
  // Handle negative durations (should not happen, but just in case)
  if (ms < 0) {
    console.warn(`[utils] Negative duration ms value: ${ms}`);
    return isRunning ? 'Just started' : 'Less than 1s';
  }
  
  // Cap unreasonably large durations
  if (ms > 31536000000) { // > 1 year
    console.warn(`[utils] Unreasonably large duration: ${ms}ms`);
  ...
  if (seconds < 60) {
    return `${seconds}s${isRunning ? ' (running)' : ''}`;
  }
  ...
  if (minutes < 60) {
    return `${minutes}m ${remainingSeconds}s${isRunning ? ' (running)' : ''}`;
  }
  ...
  if (hours < 24) {
    return `${hours}h ${remainingMinutes}m${isRunning ? ' (running)' : ''}`;
  }
  
  const days = Math.floor(hours / 24);
  const remainingHours = hours % 24;
  
  return `${days}d ${remainingHours}h${isRunning ? ' (running)' : ''}`;
} 