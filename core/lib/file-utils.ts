...
export async function loadFileContents(
  ...
  console.log(`[loadFileContents] Loading content for ${filePaths.length} files from project directory: ${projectDirectory}`);

  if (!filePaths.length) {
    console.log(`[loadFileContents] No file paths provided, returning existing contents with ${Object.keys(existingContents).length} entries`);
  ...
  console.log(`[loadFileContents] Processing ${filePathBatches.length} batches of files, up to ${BATCH_SIZE} files per batch`);
  ...
  for (let batchIndex = 0; batchIndex < filePathBatches.length; batchIndex++) {
    ...
    console.log(`[loadFileContents] Processing batch ${batchIndex + 1}/${filePathBatches.length} with ${batch.length} files`);
        ...
        if (existsSync(fullPath)) {
          ...
          try {
            ...
            if (fileStats.size > MAX_FILE_SIZE) {
              ...
              truncatedCount++;
              console.log(`[loadFileContents] Truncated large file: ${filePath} (${Math.round(fileStats.size / 1024)}KB)`);
            } else {
            ...
            console.error(`[loadFileContents] Timeout reading file ${filePath}:`, timeoutError);
            contents[filePath] = "[File read timed out - too large or locked by another process]";
            errorCount++;
          }
        } else {
          console.error(`[loadFileContents] File does not exist: ${fullPath}`);
          contents[filePath] = `[File not found: ${fullPath}]`;
          errorCount++;
        }
      } catch (error) {
        console.error(`[loadFileContents] Error reading file ${filePath}:`, error);
  ...
  console.log(`[loadFileContents] Completed loading ${filePaths.length} files. ` +
    `Success: ${successCount}, Truncated: ${truncatedCount}, Errors: ${errorCount}. ` +
    `Total entries in result map: ${Object.keys(contents).length}`);
