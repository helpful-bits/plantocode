...
/**
 * Generate simple text using a non-streaming Gemini model
 * 
 * This action is used for quick text generation tasks such as titles or summaries
 * where streaming is not needed. It uses the standard Gemini API client.
 */
export async function generateSimpleTextAction(params: { 
  ...
  const { 
    ...
    maxOutputTokens = 50,
    projectDirectory,
    taskTypeForSettings = 'title_generation' as TaskType
  } = params;
  ...
  try {
    // Use the non-streaming Gemini client
    const result = await getApiClient('gemini').sendRequest(prompt, {
      systemPrompt,
      model,
      temperature,
      maxOutputTokens,
      ...
      forceBackgroundJob: false // Explicitly disable background job
    });
    
    // Ensure we return the string data only for ActionState<string>
    return {
      isSuccess: result.isSuccess,
      message: result.message,
    ...
    return {
      isSuccess: false,
      message: error instanceof Error ? error.message : "Unknown error generating text",
      error: error instanceof Error ? error : new Error("Unknown error")
    };
  }
}
