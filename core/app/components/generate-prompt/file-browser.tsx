  ...
  return (
    <div className="space-y-4 mb-4 border rounded-lg p-6 bg-card shadow-sm">
      ...
      {!isLoading && totalFilesCount > 0 && (
        <div className="space-y-1">
          <div className={`flex items-center justify-between text-sm ${includedCount === 0 ? 'text-red-600 dark:text-red-400' : 'text-muted-foreground'} border-b pb-3`}>
            ...
            <div className={cn(filteredFiles.length === totalFilesCount && "invisible")}>
              Showing <span className="font-medium">{filteredFiles.length}</span> files
            </div>

            <div className={cn("flex gap-2", filteredFiles.length === 0 && "invisible")}>
              <Button
                type="button"
                variant="secondary"
                size="sm"
      ...
      <div className="border rounded-md bg-background/50 p-3 h-[450px] overflow-auto relative">
        ...
        <div className={cn("h-full flex flex-col items-center justify-center gap-3 text-muted-foreground p-6 absolute top-0 left-0 w-full z-10", projectDirectory ? "invisible" : "")}>
        ...
        <div className={cn("h-full flex flex-col items-center justify-center gap-3 text-muted-foreground p-6 absolute top-0 left-0 w-full z-10", !(projectDirectory && !isInitialized && isLoading) ? "invisible" : "")}>
        ...
        {/* Error loading files */}
        <div className={cn("h-full flex flex-col items-center justify-center gap-3 text-muted-foreground p-6 absolute top-0 left-0 w-full z-10", !(projectDirectory && isInitialized && fileLoadError) ? "invisible" : "")}>
        ...
        <div className={cn("h-full flex flex-col items-center justify-center gap-3 text-muted-foreground p-6 absolute top-0 left-0 w-full z-10", !(projectDirectory && isInitialized && !fileLoadError && !isLoading && Object.keys(managedFilesMap).length === 0) ? "invisible" : "")}>
          <div className="text-center">
        ...
        <div className={cn("absolute top-0 left-0 w-full h-full flex items-center justify-center bg-background/90 backdrop-blur-[1px] z-10", !(!isLoading && Object.keys(managedFilesMap).length > 0 && displayedFiles.length === 0) ? "invisible" : "")}>
