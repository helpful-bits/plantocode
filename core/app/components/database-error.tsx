...
export default function DatabaseErrorHandler() {
  ...
  const checkDatabaseHealth = async () => {
    ...
    try {
      const response = await fetch('/api/database/maintenance?action=health');
      if (response.ok) {
        const data = await response.json();
        setHealthData(data);
      } else {
        console.error('Failed to fetch database health:', response.statusText);
        setHealthData(null);
      }
    } catch (error) {
  ...
  const getRepairGuidance = () => {
    ...
    if (!healthData.fileExists) {
      return (
        <Alert className="my-4 bg-warning-background border-warning-border">
          ...
          <AlertDescription>
            <p>The database file is missing. A full reset will create a new database file.</p>
            <p className="mt-2 text-sm">Recommended action: <strong>Full Reset</strong></p>
          </AlertDescription>
        </Alert>
      );
    }
    ...
    if (healthData.fileExists && healthData.filePermissions && parseInt(healthData.filePermissions, 8) < 600) {
      return (
        <Alert className="my-4 bg-warning-background border-warning-border">
          ...
          <AlertDescription>
            <p>The database file has incorrect permissions. The repair function will attempt to fix this.</p>
            <p className="mt-2 text-sm">Recommended action: <strong>Attempt Repair</strong></p>
          </AlertDescription>
        </Alert>
      );
    }
    ...
    if (healthData.integrityStatus === 'invalid') {
      return (
        <Alert className="my-4 bg-warning-background border-warning-border">
          ...
          <AlertDescription>
            <p>The database has integrity issues which may cause unexpected behavior. The repair function will attempt to fix them.</p>
            <p className="mt-2 text-sm">If repair doesn&apos;t work, you may need to use <strong>Full Reset</strong>.</p>
          </AlertDescription>
        </Alert>
      );
    }
    ...
    return (
      <Alert className="my-4 bg-warning-background border-warning-border">
        ...
        <AlertDescription>
          <p>The database encountered an error. Try the repair function first. If that fails, a full reset may be necessary.</p>
          <p className="mt-2 text-sm">Full reset will delete all stored sessions and data.</p>
        </AlertDescription>
      </Alert>
    );
  };
        ...
        {healthData && (
          <div className="my-4 text-sm border rounded-md p-3 bg-gray-50">
            ...
            <ul className="space-y-1 pl-2">
              ...
              {healthData.fileSize && <li>File size: {(healthData.fileSize / 1024).toFixed(2)} KB</li>}
              {healthData.lastModified && <li>Last modified: {new Date(healthData.lastModified).toLocaleString()}</li>}
              {healthData.filePermissions && <li>File permissions: {healthData.filePermissions}</li>}
              <li>Integrity check: {healthData.integrityStatus || 'Unknown'}</li>
              <li>Recovery mode: {healthData.recoveryMode ? 'Active' : 'Inactive'}</li>
            </ul>
          </div>
        )}
