  ...
  return (
    ...
    >
      ...
      {job.status === 'running' && (
        <div className="mt-2 mb-1">
          ...
          <div className="flex justify-between items-center">
            {job.statusMessage && (
              <p className="text-[11px] text-blue-600 dark:text-blue-400 mt-0.5 truncate max-w-[75%]" title={job.statusMessage}>
                {job.statusMessage}
              </p>
            )}
            {job.metadata?.streamProgress && (
      ...
      <div className="min-h-[42px] max-h-[42px] overflow-hidden">
        {job.status === 'completed' && job.taskType === 'implementation_plan' && (
          <div className="text-[10px] mt-2 border-t pt-2 flex items-center gap-1.5 text-muted-foreground">
            <FileCode className="h-3.5 w-3.5 text-primary" />
            <span className="font-medium">
              {job.metadata?.sessionName ? 
                `Plan: ${job.metadata.sessionName}` : 
