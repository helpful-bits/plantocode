...
export function rowToBackgroundJob(row: any): BackgroundJob | null {
  ...
  // For implementation plan jobs specifically, ensure we have proper response handling
  if (row.task_type === 'implementation_plan' && row.status === 'completed') {
    ...
    if (isXmlContent) {
      ...
      console.log(`[Repo] Implementation plan job ${jobId} has XML-like content, preserving as-is`);
    }
    // If we have an output file path but no valid XML content in response, set file reference
    else if (outputFilePath && (!responseText || !isXmlContent)) {
      // Keep any existing response but add the file reference at the end if not already present
      if (responseText && responseText.trim() !== '' && !responseText.includes(`file: ${outputFilePath}`)) {
        responseText += `\n\n---\n\nFull implementation plan available in file: ${outputFilePath}`;
      } else {
        responseText = `Implementation plan stored in file: ${outputFilePath}`;
      }
      console.log(`[Repo] Implementation plan job ${jobId} has output file, setting appropriate reference`);
    }
    // If no file path and no XML content, set empty content message
    else if (!outputFilePath && (!responseText || responseText.trim() === '')) {
      responseText = "Implementation plan completed but no content was generated.";
      console.warn(`[Repo] Implementation plan job ${jobId} has no valid response content and no output file`);
    }
  }
  ...
  if (job.status === 'completed' && job.taskType === 'implementation_plan') {
    // 1. Check for XML content mixed with file reference messages
    // (This can happen if the response was partially streamed and then a file reference was added)
    if (job.response && job.response.includes('Content stored in file:') &&
        (job.response.includes('<implementation_plan>') ||
         job.response.includes('<steps>') ||
         job.response.includes('<step '))) {

      // Extract the XML part - we want to prioritize the actual content
      const xmlStartIndex = job.response.indexOf('<');
      if (xmlStartIndex >= 0) {
        const xmlPart = job.response.substring(xmlStartIndex);
        const filePart = job.outputFilePath ? `\n\n---\n\nFull implementation plan available in file: ${job.outputFilePath}` : '';

        // Reconstruct the response with XML content first, then file reference if needed
        job.response = xmlPart + filePart;
        console.info(`[Repo] Prioritized XML content in implementation plan response for job ${jobId}`);
      }
    }

    // 2. Handle the case where the response is JSON format
    if (job.response && (job.response.startsWith('{') || job.response.includes('"implementation_plan"'))) {
      try {
        ...
        if (parsed.implementation_plan && typeof parsed.implementation_plan === 'string') {
          // Keep only the XML content with optional file reference
          const filePart = job.outputFilePath ? `\n\n---\n\nFull implementation plan available in file: ${job.outputFilePath}` : '';
          job.response = parsed.implementation_plan + filePart;
    ...
    // 3. If we still have no response but have an outputFilePath
    if ((!job.response || job.response === '') && job.outputFilePath) {
      job.response = `Implementation plan stored in file: ${job.outputFilePath}`;
      console.info(`[Repo] Set implementation plan file reference for job ${jobId} with empty response`);
    }
  }
