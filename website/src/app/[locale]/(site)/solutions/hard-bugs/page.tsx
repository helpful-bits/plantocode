...
export async function generateMetadata({ params }: { params: Promise<{ locale: Locale }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({
    ...
    title: t['solutions.hardBugs.meta.title'],
    description: t['solutions.hardBugs.meta.description'],
  });
}
const sections = [
        ...
        <main className="flex-grow">
          <section className="py-16 sm:py-20 md:py-24 lg:py-32 px-4">
            <div className="container mx-auto max-w-5xl space-y-16">
              <Breadcrumbs items={buildSolutionBreadcrumbs(t['solutions.hardBugs.title'] || 'Hard Bugs')} />
              <header className="text-center space-y-6">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-500/10 text-amber-500 text-sm font-medium">
                  ...
                  <span>{t['solutions.hardBugs.badge'] || 'Production debugging'}</span>
                </div>
                <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold leading-tight text-foreground">
                  {t['solutions.hardBugs.title'] || 'Resolve hard bugs with preserved context'}
                </h1>
                <p className="text-lg text-foreground/80 max-w-3xl mx-auto leading-relaxed">
                  {t['solutions.hardBugs.description'] || 'PlanToCode keeps every plan, terminal session, and spoken note attached to the job you are debugging.'}
                </p>
              </header>
                ...
                {sections.map(({ icon: Icon, key, link }) => (
                  <GlassCard key={key} className="p-6">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <h2 className="text-xl font-semibold">{t[`solutions.hardBugs.sections.${key}.title`] || ''}</h2>
                    </div>
                    <p className="text-foreground/70 leading-relaxed">
                      {t[`solutions.hardBugs.sections.${key}.description`] || ''}
                    </p>
                    <LinkWithArrow href={link} className="text-sm mt-4">
                      {t[`solutions.hardBugs.sections.${key}.link`] || 'Learn more'}
                    </LinkWithArrow>
                  </GlassCard>
                ))}
              </div>
              ...
              <GlassCard className="p-8 sm:p-12 max-w-3xl mx-auto text-center" highlighted>
                <h2 className="text-2xl sm:text-3xl font-bold mb-4">
                  {t['solutions.hardBugs.cta.title'] || 'Debug Production Issues with Confidence'}
                </h2>
                <p className="text-lg text-foreground/80 mb-8 max-w-2xl mx-auto">
                  {t['solutions.hardBugs.cta.description'] || 'Preserve every investigation, reproduce every step, never lose context.'}
                </p>
                <PlatformDownloadSection location="solutions_hard_bugs" />
                <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4 text-sm text-foreground/60">
                  <LinkWithArrow href="/features/integrated-terminal">
                    {t['solutions.hardBugs.cta.links.terminal'] || 'Explore terminal persistence'}
                  </LinkWithArrow>
                  <span className="hidden sm:inline">•</span>
                  <LinkWithArrow href="/features/voice-transcription">
                    {t['solutions.hardBugs.cta.links.voice'] || 'Learn about voice notes'}
                  </LinkWithArrow>
                </div>
              </GlassCard>
            </div>
