...
function getTranslatedChangeTitle(type: keyof ChangelogEntry['changes'], t: Record<string, string>): string {
  switch (type) {
    ...
    case 'improved':
      return t['changelog.changes.improved'] ?? 'Improved';
    case 'fixed':
      return t['changelog.changes.fixed'] ?? 'Fixed';
    case 'removed':
      return t['changelog.changes.removed'] ?? 'Removed';
    default:
      return '';
  }
}
            ...
            <GlassCard key={entry.version} className="p-6 sm:p-8 transition-all duration-300 hover:border-primary/40">
              ...
              <div className="grid md:grid-cols-2 gap-6 sm:gap-8">
                <div className="space-y-6">
                  <ChangeList
                    title={getTranslatedChangeTitle('added', t)}
                    items={entry.changes.added || []}
                    type="added"
                  />

                  <ChangeList
                    title={getTranslatedChangeTitle('improved', t)}
                ...
                <div className="space-y-6">
                  <ChangeList
                    title={getTranslatedChangeTitle('fixed', t)}
                    items={entry.changes.fixed || []}
                    type="fixed"
                  />

                  <ChangeList
                    title={getTranslatedChangeTitle('removed', t)}
