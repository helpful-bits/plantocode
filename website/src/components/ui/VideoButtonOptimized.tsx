...
// Lazy load the VideoModal only when needed (when button is clicked)
const VideoModal = dynamic(() => import('./VideoModal').then(mod => ({ default: mod.VideoModal })), {
  ssr: false,
  loading: () => null,
});
...
export function VideoButtonOptimized({
  buttonText,
  ...
  const { t } = useMessages();
  const resolvedText = buttonText ?? t('videoButton.label');
