...
COMMENT ON COLUMN api_usage.tokens_input IS 'Total input tokens, including uncached and cached tokens. For accurate cost calculation, this represents all tokens sent to the provider.';
COMMENT ON COLUMN api_usage.tokens_output IS 'Total output tokens generated by the model. These tokens are always charged at the standard output rate.';
COMMENT ON COLUMN api_usage.cache_write_tokens IS 'Tokens written to cache (e.g., Anthropic''s cache_creation_input_tokens). These are charged separately from standard input tokens.';
COMMENT ON COLUMN api_usage.cache_read_tokens IS 'Tokens read from cache (e.g., OpenAI''s cached_tokens, Anthropic''s cache_read_input_tokens). These are typically charged at a discounted rate.';
COMMENT ON COLUMN api_usage.cost IS 'Total calculated cost in USD based on provider pricing and token usage. This should match provider_reported_cost when available.';
