        ...
        if let Some(metadata) = &self.metadata {
            ...
            if let Some(audio_tokens_input) = metadata.audio_tokens_input {
                if audio_tokens_input < 0 {
                    let msg = format!(
                        "Invalid metadata audio_tokens_input: {}",
                        audio_tokens_input
                    );
                    warn!("{}", msg);
                    return Err(msg);
            ...
            if let Some(audio_tokens_output) = metadata.audio_tokens_output {
                if audio_tokens_output < 0 {
                    let msg = format!(
                        "Invalid metadata audio_tokens_output: {}",
                        audio_tokens_output
                    );
                    warn!("{}", msg);
                    return Err(msg);
            ...
            if let Some(accepted_prediction_tokens) = metadata.accepted_prediction_tokens {
                if accepted_prediction_tokens < 0 {
                    let msg = format!(
                        "Invalid metadata accepted_prediction_tokens: {}",
            ...
            if let Some(rejected_prediction_tokens) = metadata.rejected_prediction_tokens {
                if rejected_prediction_tokens < 0 {
                    let msg = format!(
                        "Invalid metadata rejected_prediction_tokens: {}",
            ...
            if let Some(upstream_inference_cost) = metadata.upstream_inference_cost {
                if upstream_inference_cost < 0.0 {
                    let msg = format!(
                        "Invalid metadata upstream_inference_cost: {}",
            ...
            if let Some(prompt_tokens_details) = &metadata.prompt_tokens_details {
                for detail in prompt_tokens_details {
                    if detail.token_count < 0 {
                        let msg = format!(
                            "Invalid metadata prompt_tokens_details token_count: {}",
                            detail.token_count
                        );
                        warn!("{}", msg);
                        return Err(msg);
...
pub trait UsageExtractor {
    ...
    async fn extract_from_response_body(
        ...
        body: &[u8],
        model_id: &str,
    ) -> Result<ProviderUsage, crate::error::AppError>;
...
mod tests {
    ...
    impl UsageExtractor for MockUsageExtractor {
        async fn extract_from_response_body(
            ...
            let body_str = std::str::from_utf8(body).map_err(|e| {
                crate::error::AppError::InvalidArgument(format!("Invalid UTF-8: {}", e))
            })?;

            let json: serde_json::Value = serde_json::from_str(body_str).map_err(|e| {
                crate::error::AppError::External(format!("Failed to parse JSON: {}", e))
            })?;

            let usage = json.get("usage").ok_or_else(|| {
                crate::error::AppError::External("Missing usage field".to_string())
            })?;
            let prompt_tokens = usage
                .get("prompt_tokens")
                .and_then(|v| v.as_i64())
                .ok_or_else(|| {
                    crate::error::AppError::External("Missing prompt_tokens".to_string())
                })? as i32;
            let completion_tokens = usage
                .get("completion_tokens")
                .and_then(|v| v.as_i64())
                .ok_or_else(|| {
                    crate::error::AppError::External("Missing completion_tokens".to_string())
                })? as i32;

            let usage =
                ProviderUsage::new(prompt_tokens, completion_tokens, 0, 0, model_id.to_string());

            // Validate the extracted usage
            usage.validate().map_err(|e| {
                crate::error::AppError::InvalidArgument(format!("Validation failed: {}", e))
            })?;
    ...
    async fn test_extract_from_response_body() {
        ...
        let result = extractor
            .extract_from_response_body(response_body.as_bytes(), "test-model")
            .await;
    ...
    async fn test_extract_from_response_body_invalid_json() {
        ...
        let result = extractor
            .extract_from_response_body(invalid_body.as_bytes(), "test-model")
            .await;
