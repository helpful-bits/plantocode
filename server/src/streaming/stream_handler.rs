    ...
    S: Stream<Item = Result<web::Bytes, AppError>> + Send + Unpin + 'static,
{
    ...
    pub fn into_sse_stream(
        self,
    ) -> sse::Sse<impl Stream<Item = Result<sse::Event, actix_web::Error>>> {
        sse::Sse::from_stream(futures_util::stream::unfold(
            self,
                ...
                match handler.next_event().await {
                    Some(Ok(event)) => {
                        let sse_event = match event {
                            ...
                            StreamEvent::ContentChunk(chunk) => sse::Event::Data(sse::Data::new(
                                serde_json::to_string(&chunk).unwrap(),
                            )),
                            StreamEvent::UsageUpdate(usage) => sse::Event::Data(
    ...
    fn handle_stream_termination(
        ...
        let usage = self.final_usage.take().unwrap_or_else(|| {
            warn!(
                "Stream terminated with no usage data for request {}.",
                self.request_id
            );
        ...
        if usage.prompt_tokens == 0 && usage.completion_tokens == 0 {
            ...
            tokio::spawn(async move {
                match billing_service
                    .finalize_api_charge_with_metadata(&request_id, &user_id, usage_clone, metadata)
                    .await
                {
                    Ok(_) => {
                        tracing::debug!(
                            "Successfully marked zero-token request {} as failed",
                            request_id
                        );
                    }
                    Err(e) => {
                        tracing::error!(
                            "Failed to mark zero-token request {} as failed: {:?}",
                            request_id,
                            e
                        );
                    }
        ...
        tokio::spawn(async move {
                ...
                .await
            {
                ...
                Err(e) => {
                    tracing::error!(
                        "Failed to finalize billing for request {}: {:?}",
                        request_id,
                        e
                    );
                }
    ...
    S: Stream<Item = Result<web::Bytes, AppError>> + Send + Unpin + 'static,
{
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        if !self.start_event_sent {
            ...
            info!(
                "Sending StreamStarted event for request_id={}",
                self.request_id
            );
        ...
        match self.sse_stream.as_mut().poll_next(cx) {
            Poll::Ready(Some(Ok(sse_event))) => {
                ...
                if self.model.provider_code == "openrouter" {
                    ...
                    {
                        info!(
                            "OpenRouter potential usage-only SSE event detected: {}",
                            sse_event.data
                        );
                    }
                }
                    ...
                    Ok(value) => {
                        ...
                        if self.model.provider_code == "openrouter" {
                            if let Some(usage) = value.get("usage") {
                                info!(
                                    "OpenRouter chunk with usage data: {}",
                            ...
                            if let Some(choices) = value.get("choices") {
                                ...
                                {
                                    info!(
                                        "OpenRouter chunk with empty choices: {}",
                ...
                if let Some(error_obj) = parsed_value.get("error") {
                    ...
                    self.debug_logger
                        .log_error(&format!("Provider error: {}", error_obj));
                    ...
                    tokio::spawn(async move {
                        if let Err(fail_err) = billing_service
                            .fail_api_charge(&request_id, &user_id, &error_message)
                            .await
                        {
                            error!(
                ...
                if let Some(usage) = self.transformer.extract_usage_from_chunk(&parsed_value) {
                    if self.model.provider_code == "openrouter" {
                        info!(
                            "Extracted usage from OpenRouter chunk: prompt_tokens={}, completion_tokens={}, cost={:?}",
            ...
            Poll::Ready(Some(Err(e))) => {
                ...
                tokio::spawn(async move {
                    ...
                    {
                        error!(
                            "Could not mark request {} as failed after stream error: {:?}",
                            request_id, fail_err
                        );
                    }
                    request_tracker.remove_request(&request_id).await;
            ...
            Poll::Ready(None) => {
                if self.model.provider_code == "openrouter" {
                    info!(
                        "OpenRouter stream ended. Final usage collected: {}",
                        self.final_usage.is_some()
                    );
                    if let Some(ref usage) = self.final_usage {
                        info!(
                            "Final OpenRouter usage: prompt_tokens={}, completion_tokens={}, cost={:?}",
                            usage.prompt_tokens, usage.completion_tokens, usage.cost
                        );
                    }
                }
