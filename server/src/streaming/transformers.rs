...
impl From<StreamError> for AppError {
    fn from(stream_error: StreamError) -> Self {
        match stream_error {
            StreamError::ParseError(msg) => {
                AppError::External(format!("Stream parsing error: {}", msg))
            }
            StreamError::ProviderError(msg) => {
                AppError::External(format!("Provider error: {}", msg))
            }
            StreamError::NetworkError(msg) => AppError::External(format!("Network error: {}", msg)),
            StreamError::UsageExtractionError(msg) => {
                AppError::Internal(format!("Usage extraction failed: {}", msg))
            }
            StreamError::BillingError(msg) => {
                AppError::Internal(format!("Billing failed: {}", msg))
            }
