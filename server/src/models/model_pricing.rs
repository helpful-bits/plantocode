...
pub trait ModelPricing {
    ...
    fn calculate_total_cost(&self, usage: &ProviderUsage) -> Result<BigDecimal, String> {
        ...
        let long_context_threshold =
            parse_pricing_field_i64(pricing_info, "long_context_threshold").unwrap_or(i64::MAX);
        ...
        let input_rate = if use_long_context {
            ...
            parse_pricing_field(pricing_info, "input_per_million", &min_price, &max_price).or_else(
                |_| parse_pricing_field(pricing_info, "prompt_per_million", &min_price, &max_price),
            )
        }?;
        ...
        let output_rate = if use_long_context {
            ...
            parse_pricing_field(pricing_info, "output_per_million", &min_price, &max_price).or_else(
                |_| {
                    parse_pricing_field(
                        pricing_info,
        ...
        let base_input_tokens = usage
            ...
            .ok_or_else(|| {
                "Token arithmetic overflow: cache tokens exceed prompt tokens".to_string()
            })?;
        ...
        if usage.cache_write_tokens > 0 {
            let cache_write_rate = parse_pricing_field(
                pricing_info,
            ...
            let cache_write_cost =
                calculate_token_cost(usage.cache_write_tokens as i64, &cache_write_rate, &million)?;
            total_cost = total_cost
        ...
        if usage.cache_read_tokens > 0 {
            ...
            let cache_read_cost =
                calculate_token_cost(usage.cache_read_tokens as i64, &cache_read_rate, &million)?;
            total_cost = total_cost
        ...
        if usage.completion_tokens > 0 {
            let output_cost =
                calculate_token_cost(usage.completion_tokens as i64, &output_rate, &million)?;
            total_cost = total_cost
...
fn calculate_token_cost(
    ...
    let max_allowed = BigDecimal::from(MAX_PRICE)
        .checked_mul(million)
...
mod tests {
    ...
    fn test_cache_tokens_exceed_prompt_tokens() {
        ...
        assert!(
            err_msg.contains("cache tokens exceed prompt tokens"),
            "Expected error about cache tokens exceeding prompt tokens, got: {}",
            err_msg
        );
    }
}
