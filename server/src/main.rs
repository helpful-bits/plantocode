...
use crate::auth_stores::{Auth0StateStore, PollingStore};
use crate::config::AppSettings;
use crate::db::connection::{DatabasePools, create_dual_pools, verify_connection};
use crate::db::repositories::consent_repository::ConsentRepository;
use crate::db::{
    ApiUsageRepository, CustomerBillingRepository, DeviceRepository, ModelRepository,
    SettingsRepository, SystemPromptsRepository, UserRepository,
};
...
async fn validate_ai_model_configurations(
    ...
    let ai_settings = settings_repo
        .get_ai_model_settings()
        .await
        .map_err(|e| format!("Failed to fetch AI model settings: {}", e))?;

    let available_models = model_repo
        .get_all_with_providers()
        .await
        .map_err(|e| format!("Failed to fetch available models: {}", e))?;

    // Create a map for quick lookups
    let model_map: std::collections::HashMap<
        String,
        &crate::db::repositories::model_repository::ModelWithProvider,
    ...
    for (task_name, task_config) in &ai_settings.tasks {
        ...
        let _model = model_map.get(model_id).ok_or_else(|| {
            format!(
                "Task '{}' references non-existent model: {}",
                task_name, model_id
            )
        })?;
    ...
    for required_task in &required_tasks {
        if !ai_settings.tasks.contains_key(*required_task) {
            return Err(format!(
                "Required task '{}' is not configured",
                required_task
            ));
        }
    }
...
async fn start_reconciliation_scheduler(db_pools: DatabasePools) -> Result<(), String> {
    ...
    info!(
        "Reconciliation scheduler started successfully - will run hourly financial balance verification"
    );
...
async fn main() -> std::io::Result<()> {
    ...
    // Spawn database pool monitor for observability
    use crate::services::db_pool_monitor::DbPoolMonitor;
    let monitor = DbPoolMonitor::new(db_pools.system_pool.clone(), db_pools.user_pool.clone(), 60);
    monitor.spawn();
    log::info!("DbPoolMonitor spawned with 60s interval");
    ...
    log::info!(
        "Database AI settings verified - all configuration loaded dynamically from database"
    );
    ...
    let rate_limit_storage = match create_rate_limit_storage(
        &app_settings.rate_limit,
        &Some(app_settings.redis.url.clone()),
    )
    .await
    {
        Ok(storage) => storage,
    ...
    if let crate::middleware::rate_limiting::RateLimitStorage::Memory { .. } = &rate_limit_storage {
        ...
        tokio::spawn(async move {
            start_memory_store_cleanup_task(storage_clone, window_duration, cleanup_interval).await;
        });
    ...
    // Start request tracker cleanup task
    {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                log::info!(
                    "Request tracker cleanup completed. Active requests: {}",
                    active_count
                );
            }
        });
    ...
    let runtime_ai_config = {
        ...
        match config_handlers::load_desktop_runtime_ai_config(
            &settings_repository,
            &model_repository,
        )
        .await
    ...
    info!(
        "Client request timeout: {}s, disconnect timeout: 5s",
        app_settings_for_server.server.client_request_timeout_secs
    );
    info!(
        "CORS configured for origins: {:?}",
        app_settings_for_server.server.cors_origins
    );
    ...
    // Initialize RelaySessionStore with 24-hour TTL and 5-minute cleanup interval
    let relay_store =
        RelaySessionStore::new(chrono::Duration::hours(24), chrono::Duration::minutes(5));
    let _cleanup_handle = relay_store.clone().start_cleanup_task();
    log::info!("RelaySessionStore initialized with 24h TTL and 5min cleanup interval");

    // Initialize DeviceConnectionManager ONCE for all workers (shared via Arc)
    let device_connection_manager = web::Data::new(DeviceConnectionManager::new());
    log::info!("DeviceConnectionManager initialized and shared across all workers");
    ...
    let server = HttpServer::new(move || {
        ...
        let model_repository_for_proxy = std::sync::Arc::new(ModelRepository::new(
            std::sync::Arc::new(db_pools.system_pool.clone()),
        ));
        let settings_repository_for_proxy =
            std::sync::Arc::new(SettingsRepository::new(db_pools.system_pool.clone()));
        ...
        let consent_repository =
            std::sync::Arc::new(ConsentRepository::new(db_pools.system_pool.clone()));
        ...
        let user_repository =
            std::sync::Arc::new(UserRepository::new(db_pools.system_pool.clone())); // Auth0 lookups need system pool

        // System operations - use system pool
        let model_repository = std::sync::Arc::new(ModelRepository::new(std::sync::Arc::new(
            db_pools.system_pool.clone(),
        )));
        let settings_repository =
            std::sync::Arc::new(SettingsRepository::new(db_pools.system_pool.clone()));
        let system_prompts_repository =
            std::sync::Arc::new(SystemPromptsRepository::new(db_pools.system_pool.clone()));

        // User-specific operations - use user pool
        let customer_billing_repository =
            std::sync::Arc::new(CustomerBillingRepository::new(db_pools.user_pool.clone()));

        // Device management - use system pool for device registry
        let device_repository =
            DeviceRepository::new(std::sync::Arc::new(db_pools.system_pool.clone()));
        ...
        let public_ip_rate_limiter =
            create_ip_rate_limiter(public_rate_limit_config, rate_limit_storage.clone());

        let account_creation_rate_limiter = create_ip_rate_limiter(
            app_settings.account_creation_rate_limit.clone(),
            rate_limit_storage.clone(),
        );
        ...
        let mut app = App::new()
            ...
            .app_data(web::Data::new(ApiUsageRepository::new(
        ...
        app = app
            ...
            .service(
                web::scope("/auth")
                    .wrap(public_ip_rate_limiter.clone())
                    .configure(|cfg| {
                        configure_public_auth_routes(cfg, account_creation_rate_limiter.clone())
                    }),
            )
            ...
            .service(
                web::scope("/auth0")
                    ...
                    .route(
                        "/poll-status",
                        web::get().to(handlers::auth0_handlers::poll_auth_status),
                    )
                    .route(
                        "/finalize-login",
                        web::post().to(handlers::auth0_handlers::finalize_auth0_login),
                    ),
            )
        ...
        app
            ...
            .service(
                web::scope("/ws")
                    ...
                    .route(
                        "/device-link",
                        web::get().to(handlers::device_handlers::device_link_ws_handler),
                    )
                    .route(
                        "/events",
                        web::get().to(handlers::device_handlers::events_stream_handler),
                    ),
            )
    })
    ...
    tokio::spawn(async move {
        ...
        if should_cancel_requests {
            ...
            if active_count > 0 {
                log::info!(
                    "SIGINT: Cancelling {} active streaming requests for quick shutdown...",
                    active_count
                );
