...
pub async fn deployment_status(
    ...
    let is_authorized = if let Some(auth_value) = auth_header {
        if let Ok(auth_str) = auth_value.to_str() {
            ...
            auth_str == format!("Bearer {}", expected_token) || auth_str == expected_token
        } else {
            false
        }
    } else {
    ...
    let deployment_color =
        std::env::var("DEPLOYMENT_COLOR").unwrap_or_else(|_| "unknown".to_string());
