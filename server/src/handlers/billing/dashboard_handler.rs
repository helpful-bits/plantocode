...
pub async fn get_billing_dashboard_data_handler(
    ...
    let dashboard_data = billing_service
        .get_billing_dashboard_data(&user.user_id)
        .await?;

    info!(
        "Successfully retrieved billing dashboard data for user: {}",
        user.user_id
    );
...
pub async fn get_detailed_usage_with_summary_handler(
    ...
    let usage_summary = api_usage_repo
        .get_detailed_usage_with_summary(
            ...
            query.start_date,
            query.end_date,
            &db_pools.system_pool,
        )
        .await?;

    info!(
        "Successfully retrieved detailed usage with summary for user: {}",
        user.user_id
    );
