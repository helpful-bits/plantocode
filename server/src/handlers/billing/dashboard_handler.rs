...
pub async fn get_customer_billing_info_handler(
    ...
    debug!("Getting customer billing info for user: {}", user_id.0);
    
    let billing_info = billing_service.get_customer_billing_info(&user_id.0).await?;
    
    info!("Successfully retrieved customer billing info for user: {}", user_id.0);
...
pub async fn get_detailed_usage_with_summary_handler(
    ...
    debug!("Getting detailed usage with summary for user: {} from {} to {}", user_id.0, query.start_date, query.end_date);
    
    let usage_summary = api_usage_repo
        .get_detailed_usage_with_summary(&user_id.0, query.start_date, query.end_date)
        .await?;
    
    info!("Successfully retrieved detailed usage with summary for user: {}", user_id.0);
