...
use crate::db::repositories::credit_transaction_repository::{
...
use crate::services::audit_service::{AuditContext, AuditService};
...
async fn process_payment_completion(
    ...
    match context {
        PaymentCompletionContext::CreditPurchase {
            payment_intent,
            user_id,
        } => {
            info!(
                "Processing credit purchase for PaymentIntent: {}",
                payment_intent.id
            );

            // Extract charge ID using latest_charge (Expandable)
            let charge_id = match &payment_intent.latest_charge {
                Some(expandable_charge) => match expandable_charge {
                    Expandable::Id(charge_id) => charge_id.clone(),
                    Expandable::Object(charge) => charge.id.clone(),
                },
                None => {
                    return Err(AppError::InvalidArgument(
                        "Missing charge ID in payment intent".to_string(),
                    ));
                }
            };

            // Extract amounts from metadata instead of fetching from Stripe
            let metadata = &payment_intent.metadata.as_ref().ok_or_else(|| {
                AppError::InvalidArgument("Missing metadata in payment intent".to_string())
            })?;

            let gross_amount_str = metadata.get("gross_amount").ok_or_else(|| {
                AppError::InvalidArgument("Missing gross_amount in metadata".to_string())
            })?;
            let platform_fee_str = metadata.get("platform_fee").ok_or_else(|| {
                AppError::InvalidArgument("Missing platform_fee in metadata".to_string())
            })?;

            let gross_amount = std::str::FromStr::from_str(gross_amount_str).map_err(|_| {
                AppError::InvalidArgument("Invalid gross_amount format".to_string())
            })?;
            let platform_fee = std::str::FromStr::from_str(platform_fee_str).map_err(|_| {
                AppError::InvalidArgument("Invalid platform_fee format".to_string())
            })?;
            ...
            let metadata_json = serde_json::to_value(&payment_intent.metadata).map_err(|e| {
                AppError::InvalidArgument(format!("Failed to serialize metadata: {}", e))
            })?;
        ...
        PaymentCompletionContext::AutoTopOff {
            payment_intent,
            user,
        } => {
            info!(
                "Processing auto top-off for PaymentIntent: {}",
                payment_intent.id
            );
            ...
            let charge_id = match &payment_intent.latest_charge {
                ...
                None => {
                    return Err(AppError::InvalidArgument(
                        "Missing charge data in payment intent".to_string(),
                    ));
                }
            };

            // Common charge processing
            let (gross_amount, fee_amount, currency) =
                fetch_charge_amounts(billing_service, &charge_id).await?;
            ...
            match result {
                CreditRecordResult::Created => {
                    info!(
                        "Auto top-off credit record created - payment_intent: {}, user_id: {}, amount: {}",
                        payment_intent.id, user.id, gross_amount
                    );
                }
                CreditRecordResult::Duplicate => {
                    info!(
                        "Duplicate auto top-off credit record detected - payment_intent: {}, user_id: {}, amount: {}",
                        payment_intent.id, user.id, gross_amount
                    );
                }
            }
...
async fn fetch_charge_amounts(
    ...
    let fee_amount = match &charge.balance_transaction {
        Some(balance_transaction) => {
            BigDecimal::from(balance_transaction.fee) / BigDecimal::from(100)
        }
        None => {
            warn!(
                "No balance transaction found for charge {} - using zero fee",
                charge_id
            );
            BigDecimal::from(0)
        }
...
async fn record_credit_transaction(
        ...
        .await
    {
        Ok(updated_balance) => {
            info!(
                "Successfully processed {} - user {} new balance: ${}",
...
pub async fn stripe_webhook(
    ...
    let stripe_service = billing_service.get_stripe_service().map_err(|e| {
    ...
    let event = stripe_service
        .construct_event(body_str, stripe_signature)
        .map_err(|e| {
    ...
    info!(
        "Successfully verified Stripe webhook signature for event {} (type: {})",
    ...
    match webhook_repo.get_by_event_id(&event.id).await? {
        Some(record) => {
            match record.status.as_str() {
                "completed" => {
                    info!(
                        "Webhook event {} already processed successfully, returning 200 OK",
                        event.id
                    );
                ...
                "processing" => {
                    ...
                    if let Some(lock_expires) = record.lock_expires_at {
                        if lock_expires > Utc::now() {
                            info!(
                                "Webhook event {} is currently being processed by {}, returning 200 OK",
                                event.id,
                ...
                "failed" => {
                    ...
                    if record.retry_count >= record.max_retries {
                        warn!(
                            "Webhook event {} has permanently failed, returning 200 OK to prevent retries",
                            event.id
                        );
                        return Ok(HttpResponse::Ok().finish());
                    }
    ...
    let _webhook_record = match webhook_repo
        ...
        .await
    {
        Ok(record) => {
            info!(
                "Acquired lock for webhook event {} (worker: {})",
                event.id, worker_id
            );
            record
        }
        Err(e) => {
            error!(
                "Failed to acquire lock for webhook event {}: {}",
                event.id, e
            );
    ...
    match process_stripe_webhook_event(&event, &billing_service, &app_state).await {
        Ok(response) => {
            ...
            {
                warn!(
                    "Failed to mark webhook processing as completed for event {}: {}",
                    event.id, e
                );
            }
            info!(
                "Successfully processed webhook event {} (worker: {})",
                event.id, worker_id
            );
            Ok(response)
        }
        Err(e) => {
            ...
            error!(
                ...
                event.id, event.type_, error_message
            );
            ...
            if should_retry {
                ...
                if let Err(mark_error) = webhook_repo
                    .release_webhook_lock_with_failure(
                        &event.id,
                        &error_message,
                ...
                {
                    warn!(
                        "Failed to mark webhook as permanently failed for event {}: {}",
                        event.id, mark_error
                    );
                }
            }
...
async fn process_stripe_webhook_event(
    ...
    info!(
        "Processing Stripe webhook event {} of type {}",
        event.id, event.type_
    );
    ...
    match event.type_.as_str() {
        ...
        EVENT_PAYMENT_INTENT_SUCCEEDED => {
            ...
            let payment_type = payment_intent
                .metadata
                .as_ref()
            ...
            info!(
                "Payment intent {} type: {} - processing",
                payment_intent.id, payment_type
            );
                ...
                if let Some(metadata) = &payment_intent.metadata {
                    if let Some(user_id_str) = metadata.get("user_id") {
                        if let Ok(user_id) = Uuid::parse_str(user_id_str) {
                            let audit_context =
                                crate::services::audit_service::AuditContext::new(user_id);
                            ...
                            {
                                warn!(
                                    "Failed to log payment success audit for user {}: {}",
                                    user_id, e
                                );
                            }
                        }
            ...
            match payment_type {
                "credit_purchase" => {
                    let user_id_str = payment_intent
                        .metadata
                        .as_ref()
                    ...
                    info!(
                        "Processing credit purchase for payment intent {} - user_id: {}, amount: {}",
                    ...
                    let user_id = match Uuid::parse_str(&user_id_str) {
                        ...
                        Err(e) => {
                            error!(
                                "Invalid user_id UUID format in payment intent {}: '{}' - {}",
                    ...
                    match process_payment_completion(context, billing_service).await {
                        Ok(_) => {
                            info!(
                                "Successfully processed credit purchase for payment intent {} - user_id: {}, amount: {}",
                                payment_intent_id, user_id_str, amount
                            );
                        }
                        Err(e) => {
                            error!(
                                "Failed to process credit purchase for payment intent {} - user_id: {}, amount: {} - Error: {}",
                ...
                "auto_topoff" => {
                    ...
                    info!(
                        "Processing auto top-off for payment intent {} (PRIMARY handler)",
                        payment_intent_id
                    );

                    let customer_id = payment_intent.customer.as_ref().ok_or_else(|| {
                        AppError::InvalidArgument("No customer ID in payment intent".to_string())
                    })?;
                    ...
                    match process_payment_completion(context, billing_service).await {
                        Ok(_) => {
                            info!(
                                "Successfully processed auto top-off for payment intent {}",
                                payment_intent_id
                            );
                        }
                        Err(e) => {
                            error!(
                                "Failed to process auto top-off for payment intent {}: {}",
                                payment_intent_id, e
                            );
                            crate::utils::admin_alerting::send_payment_processing_error_alert(
                                &payment_intent_id,
                ...
                _ => {
                    ...
                    warn!(
                        "Unknown payment type '{}' for payment intent {} - attempting credit purchase processing",
                        payment_type, payment_intent_id
                    );

                    if let Some(user_id_str) = payment_intent
                        .metadata
                        .as_ref()
                        .and_then(|metadata| metadata.get("user_id"))
                        .map(|v| v.as_str())
                    {
                        if let Ok(user_id) = Uuid::parse_str(user_id_str) {
                            ...
                            match process_payment_completion(context, billing_service).await {
                                Ok(_) => {
                                    info!(
                                        "Successfully processed unknown payment type as credit purchase for payment intent {}",
                                        payment_intent_id
                                    );
                                }
                                Err(e) => {
                                    warn!(
                                        "Failed to process unknown payment type for payment intent {}: {} - continuing webhook processing",
                                        payment_intent_id, e
                                    );
                                }
                            }
                            ...
                            warn!(
                                "Invalid user_id in unknown payment type for payment intent {}: {}",
                                payment_intent_id, user_id_str
                            );
                        }
                    } else {
                        warn!(
                            "No user_id found in metadata for unknown payment type payment intent {}",
                            payment_intent_id
                        );
                    }
                }
        ...
        EVENT_PAYMENT_METHOD_ATTACHED => {
            ...
            let payment_method: PaymentMethod =
                serde_json::from_value(event.data["object"].clone()).map_err(|e| {
                    AppError::InvalidArgument(format!("Failed to parse payment method: {}", e))
                })?;
        ...
        EVENT_CUSTOMER_DEFAULT_SOURCE_UPDATED => {
            ...
            let customer: Customer =
                serde_json::from_value(event.data["object"].clone()).map_err(|e| {
                    AppError::InvalidArgument(format!("Failed to parse customer: {}", e))
                })?;
        ...
        EVENT_INVOICE_CREATED | EVENT_INVOICE_FINALIZED => {
            ...
            if is_auto_topoff {
                info!(
                    "Processing auto top-off invoice event: {} for invoice: {}",
                    event.type_, invoice_id
                );
            } else {
                info!(
                    "Processing regular invoice event: {} for invoice: {}",
                    event.type_, invoice_id
                );
            }
            ...
            if event.type_ == EVENT_INVOICE_FINALIZED {
                if let Some(amount_due) = event
                    .data
                    .get("object")
                    .and_then(|o| o.get("amount_due"))
                    .and_then(|a| a.as_i64())
                {
                    if amount_due == 0 {
                        error!(
                            "ALERT: $0 invoice detected! Event ID: {} - Invoice ID: {}",
                            event.id, invoice_id
                        );

                        // Send admin alert for $0 invoice
                        let alerting_service =
                            crate::utils::admin_alerting::AdminAlertingService::new();
                        let alert = crate::utils::admin_alerting::AdminAlert::new(
                            ...
                            format!(
                                "A $0 invoice was finalized (Event: {}, Invoice: {}). Please investigate in Stripe Dashboard.",
                                event.id, invoice_id
                            ),
                        );
        ...
        EVENT_INVOICE_PAID => {
            ...
            if is_auto_topoff {
                ...
                let customer_id = invoice.customer.as_ref().ok_or_else(|| {
                    AppError::InvalidArgument("Invoice missing customer ID".to_string())
                })?;
                ...
                info!(
                    "Found user {} for auto top-off invoice {}",
                    user.id, invoice.id
                );
                ...
                match result {
                    CreditRecordResult::Created => {
                        info!(
                            "Successfully processed auto top-off invoice {} for user {} - added ${} credits",
                            invoice.id, user.id, gross_amount
                        );
                    }
                    CreditRecordResult::Duplicate => {
                        info!(
                            "Duplicate auto top-off invoice {} for user {} detected - credits already processed",
                            invoice.id, user.id
                        );
                    }
                }
                ...
                info!(
                    "Invoice {} is not an auto top-off, skipping credit processing",
                    invoice.id
                );
            }
        }
        _ => {
            info!(
                "Ignoring Stripe event type: {} - handled by Customer Portal",
                event.type_
            );
        }
    }
...
async fn handle_payment_method_attached(
    ...
    let customer_id = match &payment_method.customer {
        ...
        None => {
            warn!(
                "Payment method {} has no associated customer, skipping audit",
                payment_method.id
            );
            return Ok(());
        }
    ...
    let user_repo = crate::db::repositories::user_repository::UserRepository::new(
        billing_service.get_system_db_pool(),
    );
    let user = match user_repo.get_by_stripe_customer_id(&customer_id).await {
        Ok(user) => user,
        Err(_) => {
            warn!(
                "Could not find user for customer {} with attached payment method {}",
                customer_id, payment_method.id
            );
            return Ok(());
        }
    };
    ...
    if let Err(e) = audit_service.log_event(&audit_context, audit_event).await {
        warn!(
            "Failed to log payment method attachment audit for user {}: {}",
            user.id, e
        );
    }

    info!(
        "Audit: Payment method {} of type {:?} attached to customer {} (user {}) via Stripe Customer Portal",
        payment_method.id, payment_method.type_, customer_id, user.id
    );
...
async fn handle_payment_method_detached(
    ...
    let user = match user_repo.get_by_stripe_customer_id(&customer_id).await {
        ...
        Err(_) => {
            warn!(
                "Could not find user for customer {} with detached payment method {}",
                customer_id, payment_method.id
            );
            return Ok(());
        }
    ...
    if let Err(e) = audit_service.log_event(&audit_context, audit_event).await {
        warn!(
            "Failed to log payment method detachment audit for user {}: {}",
            user.id, e
        );
    }

    info!(
        "Audit: Payment method {} of type {:?} detached from customer {} (user {}) via Stripe Customer Portal",
        payment_method.id, payment_method.type_, customer_id, user.id
    );
...
async fn handle_customer_default_source_updated(
    ...
    let user = match user_repo.get_by_stripe_customer_id(&customer.id).await {
        ...
        Err(_) => {
            warn!(
                "Could not find user for customer {} with updated default source",
                customer.id
            );
            return Ok(());
        }
    ...
    match default_payment_method_id {
        Some(pm_id) => {
            info!(
                "Customer {} (user {}) default payment method changed to {}",
            ...
            if let Err(e) = audit_service.log_event(&audit_context, audit_event).await {
                warn!(
                    "Failed to log default payment method update audit for user {}: {}",
                    user.id, e
                );
            }

            info!(
                "Audit: Customer {} (user {}) default payment method changed to {} via Stripe Customer Portal",
                customer.id, user.id, pm_id
            );
        }
        None => {
            info!(
                "Customer {} (user {}) default payment method was removed",
                customer.id, user.id
            );

            let audit_event = crate::services::audit_service::AuditEvent::new(
                "default_payment_method_removed",
                "customer",
            )
            .with_entity_id(customer.id.clone())
            .with_metadata(serde_json::json!({
            ...
            if let Err(e) = audit_service.log_event(&audit_context, audit_event).await {
                warn!(
                    "Failed to log default payment method removal audit for user {}: {}",
                    user.id, e
                );
            }

            info!(
                "Audit: Customer {} (user {}) default payment method removed via Stripe Customer Portal",
                customer.id, user.id
            );
        }
    }
...
async fn handle_checkout_session_completed(
    ...
    if let Some(presentment_details) = &session.presentment_details {
        info!(
            "Checkout session {} completed with Adaptive Pricing: {} {} (originally USD)",
            session.id,
    ...
    match session.mode {
        CheckoutSessionMode::Payment => {
            ...
            if let Some(payment_intent_id) = session.payment_intent.as_ref() {
                ...
                let payment_intent = stripe_service
                    ...
                    .await
                    .map_err(|e| {
                        AppError::External(format!("Failed to retrieve payment intent: {}", e))
                    })?;
                ...
                let user_id_str = payment_intent
                    ...
                    .ok_or_else(|| {
                        AppError::InvalidArgument(
                            "Missing user_id in payment intent metadata".to_string(),
                        )
                    })?;

                let user_id = Uuid::parse_str(user_id_str).map_err(|e| {
                    AppError::InvalidArgument(format!("Invalid user_id UUID: {}", e))
                })?;
                    ...
                    if let Some(customer_id) = &payment_intent.customer {
                        ...
                        match stripe_service.get_customer(customer_id).await {
                            Ok(customer) => {
                                ...
                                if !has_default {
                                    info!(
                                        "Customer {} has no default payment method, setting {} as default",
                                        customer_id, payment_method_id
                                    );
                                    let idempotency_key = format!(
                                        "set_default_pm_checkout_{}_{}",
                                        session.id, payment_method_id
                                    );
                                    ...
                                    {
                                        warn!(
                                            "Failed to set default payment method after checkout: {}",
                                            e
                                        );
                                        // Non-critical error - continue processing
                                    } else {
                                        info!(
                                            "Successfully set default payment method {} for customer {}",
                                            payment_method_id, customer_id
                                        );
                                    }
                                } else {
                                    info!(
                                        "Customer {} already has a default payment method, preserving existing default",
                                        customer_id
                                    );
                                }
                            }
                            Err(e) => {
                                warn!(
                                    "Failed to retrieve customer {} to check default PM: {}",
                                    customer_id, e
                                );
                                // Non-critical error - continue processing
                            }
        ...
        CheckoutSessionMode::Setup => {
            info!(
                "Setup mode checkout session completed successfully: {}",
                session.id
            );
            ...
            if let Some(setup_intent_id) = session.setup_intent.as_ref() {
                ...
                let setup_intent = stripe_service
                    ...
                    .await
                    .map_err(|e| {
                        AppError::External(format!("Failed to retrieve setup intent: {}", e))
                    })?;

                // Process the saved payment method
                billing_service
                    .process_saved_payment_method(&setup_intent)
                    .await?;
                info!(
                    "Successfully processed saved payment method from setup intent: {}",
                    setup_intent_id
                );
            } else {
