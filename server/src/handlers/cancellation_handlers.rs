...
pub async fn cancel_request_handler(
    ...
    info!(
        "Cancellation request received for request_id: {} by user: {}",
        request_id, user_id
    );
    ...
    if tracked_request.user_id != user_id {
        warn!(
            ...
            user_id, request_id, tracked_request.user_id
        );
        return Err(AppError::Forbidden(
            "You don't have permission to cancel this request".to_string(),
        ));
    }
            ...
            if let Some(response_id) = tracked_request.openai_response_id {
                ...
                let openai_api_key =
                    app_settings
                        ...
                        .as_ref()
                        .ok_or_else(|| {
                            AppError::Configuration("OpenAI API key not configured".to_string())
                        })?;

                let cancel_url =
                    format!("https://api.openai.com/v1/responses/{}/cancel", response_id);
                ...
                let response = client
                    ...
                    .await
                    .map_err(|e| {
                        AppError::External(format!("Failed to send cancellation request: {}", e))
                    })?;

                if response.status().is_success() {
                    info!(
                        "Successfully cancelled OpenAI response {} for request {}",
                ...
                if let Err(e) = billing_service
                    .fail_api_charge(request_id, &user_id, "Cancelled by user")
                    .await
                {
                    warn!(
                        "Failed to refund charge for cancelled request {}: {}",
                        request_id, e
                    );
                }
                ...
                // Fail the charge to refund the estimate and release reservation
                if let Err(e) = billing_service
                    .fail_api_charge(request_id, &user_id, "Cancelled before processing")
                    .await
                {
                    warn!(
                        "Failed to refund charge for cancelled request {}: {}",
                ...
                Ok(HttpResponse::Ok().json(CancelRequestResponse {
                    success: true,
                    message: format!(
                        "Request {} has been cancelled before processing",
                        request_id
                    ),
                }))
            }
            ...
            if tracked_request.is_streaming {
                ...
                match request_tracker.cancel_request(request_id).await {
                    Ok(true) => {
                        ...
                        info!(
                            "Successfully cancelled streaming request {} for provider: {}",
                            request_id, tracked_request.provider
                        );
                    ...
                    Ok(false) => {
                        ...
                        warn!(
                            "Streaming request {} for provider {} has no cancellation token - will complete in background",
                            request_id, tracked_request.provider
                        );
                ...
                info!(
                    "Non-streaming request {} cannot be cancelled but will complete in background for accurate billing",
                    request_id
                );
...
pub async fn get_request_status_handler(
    ...
    if tracked_request.user_id != user_id {
        return Err(AppError::Forbidden(
            "You don't have permission to view this request".to_string(),
        ));
    }
