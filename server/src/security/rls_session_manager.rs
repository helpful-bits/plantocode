...
impl RLSSessionManager {
    ...
    pub async fn get_connection_with_user_context(
        ...
        debug!(
            "RLS: Getting connection for user {} (request: {})",
            user_id, req_id
        );

        // Acquire connection from pool
        let mut conn = AcquireRetry::acquire_with_retry(&self.pool, 2, 100)
            .await
            .map_err(|e| {
                error!(
                    "RLS: Failed to acquire connection from pool for user {} (request: {}): {}",
                    user_id, req_id, e
                );
        ...
        sqlx::query("SELECT set_config('application_name', format('plantocode-user:%s', pg_backend_pid()::text), true)")
            ...
            .await
            .map_err(|e| {
                error!("RLS: Failed to set application_name tag (conn: {}): {}", connection_id, e);
                AppError::Database(format!("Failed to set application_name tag: {}", e))
            })?;
        ...
        if let Err(e) = self.reset_connection_state(&mut conn, &connection_id).await {
            error!(
                "RLS: Failed to reset connection state for user {} (request: {}, conn: {}): {}",
        ...
        if let Err(e) = self
            .set_user_context(&mut conn, user_id, &connection_id, &req_id)
            .await
        {
            error!(
                "RLS: Failed to set user context for user {} (request: {}, conn: {}): {}",
                user_id, req_id, connection_id, e
            );
            tokio::spawn({
                let manager = self.clone();
        ...
        // Validate RLS setup
        if let Err(e) = self
            .validate_rls_setup(&mut conn, user_id, &connection_id, &req_id)
            .await
        {
            error!(
                "RLS: RLS validation failed for user {} (request: {}, conn: {}): {}",
                user_id, req_id, connection_id, e
            );
            tokio::spawn({
                let manager = self.clone();
        ...
        self.track_connection_state(connection_id.clone(), user_id)
            .await;

        let duration = start_time.elapsed();
        debug!(
            "RLS: Successfully configured connection for user {} in {:?} (request: {}, conn: {})",
    ...
    async fn reset_connection_state(
        ...
        .map_err(|e| {
            error!(
                "RLS: Failed to reset session variables (conn: {}): {}",
                connection_id, e
            );
        ...
        debug!(
            "RLS: Connection state reset completed (conn: {})",
            connection_id
        );
        Ok(())
    }
    ...
    async fn set_user_context(
        ...
        debug!(
            "RLS: Setting user context for user {} (conn: {}, request: {})",
        ...
        info!(
            "RLS: User context successfully configured for user {} (conn: {}, request: {})",
            user_id, connection_id, request_id
        );
        Ok(())
    }
    ...
    async fn validate_rls_setup(
        ...
        debug!(
            "RLS: Validating RLS setup for user {} (conn: {}, request: {})",
            expected_user_id, connection_id, request_id
        );

        // Test the RLS helper function that policies depend on
        let validation_result =
            sqlx::query_as::<_, (Option<String>,)>("SELECT get_current_user_id()::text")
                .fetch_one(&mut *conn)
                .await;

        match validation_result {
            Ok((Some(user_id_str),)) => match user_id_str.parse::<Uuid>() {
                Ok(actual_user_id) if actual_user_id == expected_user_id => {
                    debug!(
                        "RLS: Validation successful - user context matches expected {} (conn: {}, request: {})",
                        expected_user_id, connection_id, request_id
                    );
                }
                Ok(actual_user_id) => {
                    error!(
                        "RLS: CRITICAL - User context mismatch! Expected: {}, Got: {} (conn: {}, request: {})",
                ...
                Err(e) => {
                    error!(
                        "RLS: CRITICAL - Invalid user ID format in session variable: '{}' (conn: {}, request: {}): {}",
            ...
            Ok((None,)) => {
                error!(
                    "RLS: CRITICAL - get_current_user_id() returned NULL! RLS policies will fail silently (conn: {}, request: {})",
            ...
            Err(e) => {
                error!(
                    "RLS: CRITICAL - Failed to validate RLS setup (conn: {}, request: {}): {}",
        ...
        match user_test_result {
            Ok((count,)) if count > 0 => {
                debug!(
                    "RLS: User query validation successful (conn: {}, request: {})",
                    connection_id, request_id
                );
            }
            Ok((0,)) => {
                warn!(
                    "RLS: User not found in database for ID {} (conn: {}, request: {})",
                    expected_user_id, connection_id, request_id
                );
            ...
            Ok((count,)) => {
                warn!(
                    "RLS: Unexpected count value {} for user query validation (conn: {}, request: {})",
            ...
            Err(e) => {
                error!(
                    "RLS: User query validation failed (conn: {}, request: {}): {}",
                    connection_id, request_id, e
                );
                return Err(AppError::Database(format!(
        ...
        info!(
            "RLS: Complete validation successful for user {} (conn: {}, request: {})",
            expected_user_id, connection_id, request_id
        );
        Ok(())
    }
    ...
    pub async fn validate_connection_context(
        ...
        match validation_result {
            Ok((Some(user_id_str),)) => match user_id_str.parse::<Uuid>() {
                ...
                Ok(actual_user_id) => {
                    error!(
                        "RLS: Connection context mismatch! Expected: {}, Got: {}",
                        expected_user_id, actual_user_id
                    );
                    tokio::spawn({
                        let manager = self.clone();
                ...
                Err(_) => {
                    error!(
                        "RLS: Invalid user ID in connection context: {}",
                        user_id_str
                    );
                    Err(AppError::Auth(
                        "Invalid user ID in connection context".to_string(),
                    ))
                }
            },
            Ok((None,)) => {
...
mod tests {
    ...
    async fn test_user_context_setup() {
        ...
        let result = manager
            .get_connection_with_user_context(user_id, None)
            .await;
