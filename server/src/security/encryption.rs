...
pub fn encrypt(data: &str, key: &[u8]) -> Result<Vec<u8>, AppError> {
    if key.len() != 32 {
        return Err(AppError::Configuration("Encryption key must be 32 bytes".to_string()));
    }
...
pub fn decrypt(encrypted_data: &[u8], key: &[u8]) -> Result<String, AppError> {
    if key.len() != 32 {
        return Err(AppError::Configuration("Decryption key must be 32 bytes".to_string()));
    }
