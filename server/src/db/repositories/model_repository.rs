...
static DEFAULT_PRICING: Lazy<serde_json::Value> = Lazy::new(|| serde_json::json!({}));
...
impl ModelRepository {
    ...
    pub async fn get_all_with_providers(&self) -> AppResult<Vec<ModelWithProvider>> {
        ...
        let mappings = self
            .mapping_service
            .get_all_models_with_mappings_all_providers()
            .await
            .map_err(|e| {
                AppError::Database(format!("Failed to fetch models with mappings: {}", e))
            })?;

        let result: Vec<ModelWithProvider> = mappings
            .into_iter()
            .map(|mapping| ModelWithProvider {
        ...
        info!(
            "Retrieved {} active models with provider information",
            result.len()
        );
        Ok(result)
    }
    ...
    pub async fn find_by_id_with_provider(&self, id: &str) -> AppResult<Option<ModelWithProvider>> {
        ...
        .await
        .map_err(|e| {
            AppError::Database(format!("Failed to fetch model info for ID {}: {}", id, e))
        })?;
        ...
        match model_info {
            Some(info) => {
                ...
                match self
                    .mapping_service
                    .get_model_with_mapping(id, &info.provider_code)
                    .await
                {
                    Ok(mapping) => {
                        ...
                        info!(
                            "Found model: {} from provider {}",
                            result.name, result.provider_name
                        );
                        Ok(Some(result))
                    }
                    Err(_) => {
    ...
    #[instrument(skip(self))]
    pub async fn get_by_provider_code(
        &self,
        provider_code: &str,
    ) -> AppResult<Vec<ModelWithProvider>> {
        info!("Fetching models for provider: {}", provider_code);

        let mappings = self
            .mapping_service
            .get_all_models_with_mappings(provider_code)
            .await
            .map_err(|e| {
                AppError::Database(format!(
                    "Failed to fetch models with mappings for provider {}: {}",
                    provider_code, e
                ))
            })?;
    ...
    pub async fn get_by_type(&self, model_type: &str) -> AppResult<Vec<ModelWithProvider>> {
        ...
        let mappings = self
            .mapping_service
            .get_models_by_type_with_mappings(model_type)
            .await
            .map_err(|e| {
                AppError::Database(format!(
                    "Failed to fetch models of type {} with mappings: {}",
                    model_type, e
                ))
            })?;
    ...
    pub async fn update_model_pricing(
        ...
        let result = sqlx::query(query)
            ...
            .map_err(|e| {
                AppError::Database(format!(
                    "Failed to update model pricing for {}: {}",
                    model_id, e
                ))
            })?;
    ...
    #[instrument(skip(self))]
    pub async fn find_by_id_with_mapping(
        &self,
        id: &str,
        provider_code: &str,
    ) -> AppResult<Option<ModelWithMapping>> {
        info!(
            "Fetching model by ID with mapping: {} for provider: {}",
            id, provider_code
        );

        match self
            .mapping_service
            .get_model_with_mapping(id, provider_code)
            .await
        {
            Ok(mapping) => {
                info!(
                    "Found model with mapping: {} -> {}",
                    id, mapping.resolved_model_id
                );
                Ok(Some(mapping))
            }
            Err(_) => {
                info!(
                    "No model found with mapping for ID: {} with provider: {}",
                    id, provider_code
                );
                Ok(None)
            }
    ...
    #[instrument(skip(self))]
    pub async fn find_provider_model_id(
        &self,
        internal_model_id: &str,
        provider_code: &str,
    ) -> AppResult<Option<String>> {
        info!(
            "Finding provider model ID for internal model: {} with provider: {}",
            internal_model_id, provider_code
        );

        match self
            .mapping_service
            .resolve_model_id(internal_model_id, provider_code)
            .await
        {
            Ok(resolved_id) => {
                info!(
                    "Found provider model ID: {} for internal model: {} with provider: {}",
                    resolved_id, internal_model_id, provider_code
                );
                Ok(Some(resolved_id))
            }
            Err(_) => {
                info!(
                    "No provider model ID found for internal model: {} with provider: {}",
                    internal_model_id, provider_code
                );
                Ok(None)
            }
