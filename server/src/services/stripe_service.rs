...
impl StripeService {
    ...
    async fn make_stripe_request_with_idempotency(
        ...
        headers.insert(
            ...
            HeaderValue::from_str(&format!("Bearer {}", self.secret_key)).map_err(|_| {
                StripeServiceError::Configuration("Invalid secret key format".to_string())
            })?,
        );
        headers.insert(
            CONTENT_TYPE,
            ...
            if let Some(key) = idempotency_key {
                headers.insert(
                    ...
                    HeaderValue::from_str(key).map_err(|_| {
                        StripeServiceError::Configuration(
                            "Invalid idempotency key format".to_string(),
                        )
                    })?,
                );
            }
        ...
        let stripe_version =
            std::env::var("STRIPE_API_VERSION").unwrap_or_else(|_| "2024-06-20".to_string());
        headers.insert(
            "Stripe-Version",
            HeaderValue::from_str(&stripe_version).map_err(|_| {
                StripeServiceError::Configuration("Invalid Stripe version format".to_string())
            })?,
        );
        ...
        let response = request.send().await.map_err(|e| {
            StripeServiceError::Configuration(format!("HTTP request failed: {}", e))
        })?;
        ...
        if !response.status().is_success() {
            ...
            if let Ok(error_json) = serde_json::from_str::<serde_json::Value>(&error_text) {
                if let Some(error_obj) = error_json.get("error") {
                    ...
                    let error_code = error_obj.get("code").and_then(|c| c.as_str()).unwrap_or("");

                    error!(
                        "Stripe API error - Status: {}, Type: {}, Code: {}, Message: {}",
                        status, error_type, error_code, error_message
                    );

                    // Return appropriate error type based on the error
                    return match error_type {
                        "card_error" | "payment_method_error" => Err(
                            StripeServiceError::PaymentProcessing(error_message.to_string()),
                        ),
                        "invalid_request_error" | "api_error" => {
                            Err(StripeServiceError::StripeApi(format!(
                                "{}: {}",
                                error_code, error_message
                            )))
                        }
                        _ => Err(StripeServiceError::Configuration(format!(
                            "Stripe API error: {}",
                            error_message
                        ))),
                    };
            ...
            error!(
                "Stripe API error - Status: {}, Response: {}",
                status, error_text
            );
            return Err(StripeServiceError::StripeApi(format!(
                "HTTP {}: {}",
                status, error_text
            )));
        }
        ...
        let response_json: serde_json::Value = response.json().await.map_err(|e| {
            StripeServiceError::Configuration(format!("Failed to parse response: {}", e))
        })?;
    ...
    pub fn verify_webhook_signature(
        ...
        let timestamp_int: i64 = timestamp.parse().map_err(|e| {
            StripeServiceError::WebhookVerification(format!("Invalid timestamp format: {}", e))
        })?;
        ...
        if time_diff > 300 {
            ...
            return Err(StripeServiceError::WebhookVerification(format!(
                "Webhook timestamp outside allowed window: webhook is {} seconds old (maximum 300 seconds allowed)",
                time_diff
            )));
        }
        ...
        for signature in signatures {
            let decoded = hex::decode(signature).map_err(|e| {
                StripeServiceError::WebhookVerification(format!("Invalid signature format: {}", e))
            })?;
            let expected_bytes = hex::decode(&expected_signature).map_err(|e| {
                StripeServiceError::WebhookVerification(format!(
                    "Invalid expected signature format: {}",
                    e
                ))
            })?;

            if decoded.len() == expected_bytes.len()
                && decoded.ct_eq(&expected_bytes).unwrap_u8() == 1
            {
                return Ok(());
            }
        }
    ...
    pub fn parse_webhook_event(
        ...
        let event: stripe_types::Event = serde_json::from_str(payload).map_err(|e| {
    ...
    pub fn process_stripe_webhook_event(
        ...
        info!(
            "Processing Stripe webhook event {} (type: {}) with idempotency key: {}",
    ...
    pub async fn search_customers(
        &self,
        query: &str,
    ) -> Result<SearchResult<Customer>, StripeServiceError> {
        let encoded_query = urlencoding::encode(query);
        let response = self
            .make_stripe_request_with_idempotency(
                reqwest::Method::GET,
        ...
        let search_result: SearchResult<Customer> =
            serde_json::from_value(response).map_err(|e| {
                StripeServiceError::Configuration(format!(
        ...
        info!(
            "Customer search for query '{}' returned {} results",
            query,
    ...
    pub async fn search_customer_by_user_id(
        &self,
        user_id: &Uuid,
    ) -> Result<Option<Customer>, StripeServiceError> {
    ...
    pub async fn create_customer(
        ...
        let customer: stripe_types::Customer = serde_json::from_value(response).map_err(|e| {
            StripeServiceError::Configuration(format!("Failed to parse customer response: {}", e))
        })?;
    ...
    pub async fn create_billing_portal_session(
        ...
        let session: stripe_types::BillingPortalSession = serde_json::from_value(response)
            .map_err(|e| {
                StripeServiceError::Configuration(format!(
                    "Failed to parse billing portal session response: {}",
                    e
                ))
            })?;
    ...
    pub async fn get_charge(
        &self,
        charge_id: &str,
    ) -> Result<stripe_types::Charge, StripeServiceError> {
        let response = self
            .make_stripe_request_with_idempotency(
                reqwest::Method::GET,
            ...
            .await?;
        let charge: stripe_types::Charge = serde_json::from_value(response).map_err(|e| {
            StripeServiceError::Configuration(format!("Failed to parse charge response: {}", e))
        })?;
        Ok(charge)
    }
    ...
    pub async fn list_payment_methods(
        &self,
        customer_id: &str,
    ) -> Result<Vec<stripe_types::PaymentMethod>, StripeServiceError> {
        let response = self
            .make_stripe_request_with_idempotency(
                reqwest::Method::GET,
        ...
        let payment_methods_list: stripe_types::PaymentMethodList =
            serde_json::from_value(response).map_err(|e| {
                StripeServiceError::Configuration(format!(
        ...
        info!(
            "Retrieved {} payment methods for customer: {}",
            payment_methods_list.data.len(),
            customer_id
        );
    ...
    pub async fn detach_payment_method(
    ...
    ) -> Result<stripe_types::PaymentMethod, StripeServiceError> {
        ...
        let payment_method: stripe_types::PaymentMethod = serde_json::from_value(response)
            .map_err(|e| {
                StripeServiceError::Configuration(format!(
                    "Failed to parse payment method response: {}",
                    e
                ))
            })?;
    ...
    pub async fn create_product_and_price(
        ...
        let product: stripe_types::Product =
            serde_json::from_value(product_response).map_err(|e| {
                StripeServiceError::Configuration(format!(
        ...
        if let Some(recurring_interval) = interval {
            price_data["recurring[interval]"] =
                serde_json::Value::String(recurring_interval.to_string());
        }
        ...
        let price: stripe_types::Price = serde_json::from_value(price_response).map_err(|e| {
            StripeServiceError::Configuration(format!("Failed to parse price response: {}", e))
        })?;

        info!(
            "Created product {} and price {} for {}",
            product.id, price.id, product_name
        );
        Ok((product, price))
    }
    ...
    pub async fn create_setup_intent(
        ...
        let setup_intent: stripe_types::SetupIntent =
            serde_json::from_value(response).map_err(|e| {
                StripeServiceError::Configuration(format!(
                    "Failed to parse setup intent response: {}",
                    e
                ))
            })?;
    ...
    pub async fn list_invoices(
        ...
        let mut query_params = vec![("customer".to_string(), customer_id.to_string())];
        ...
        if let Some(starting_after_value) = starting_after {
            query_params.push((
                ...
                starting_after_value.to_string(),
            ));
        }
    ...
    pub async fn create_checkout_session(
        ...
        match mode_str {
            "payment" => {
                ...
                payment_intent_data.insert(
                    ...
                    serde_json::Value::String("off_session".to_string()),
                );
                ...
                if !payment_intent_data.is_empty() {
                    session_data["payment_intent_data"] =
                        serde_json::Value::Object(payment_intent_data);
                }
            }
            "setup" => {
                if !metadata.is_empty() {
                    ...
                    setup_intent_data.insert(
                        "metadata".to_string(),
                        serde_json::Value::Object(metadata_obj),
                    );
                    session_data["setup_intent_data"] =
                        serde_json::Value::Object(setup_intent_data);
                }
            }
            _ => {}
        }
        ...
        let session: stripe_types::CheckoutSession =
            serde_json::from_value(response).map_err(|e| {
                StripeServiceError::Configuration(format!(
        ...
        info!(
            "Created Checkout Session: {} for customer: {}",
    ...
    pub async fn create_invoice_for_auto_topoff(
        ...
        info!(
            "Creating invoice for auto top-off - customer: {}, net: {} cents, fee: {} cents, total: {} {}",
        ...
        let mut invoice: stripe_types::Invoice =
            serde_json::from_value(invoice_response).map_err(|e| {
                StripeServiceError::Configuration(format!(
        ...
        let credits_item_id = credits_item
            .get("id")
            .and_then(|v| v.as_str())
            .unwrap_or("unknown");
        info!(
            "Created credits invoice item: {} attached to invoice: {}",
            credits_item_id, invoice.id
        );
        ...
        if fee_amount_cents > 0 {
            ...
            let fee_item_id = fee_item
                .get("id")
                .and_then(|v| v.as_str())
                .unwrap_or("unknown");
            info!(
                "Created fee invoice item: {} attached to invoice: {}",
                fee_item_id, invoice.id
            );
        }
        ...
        invoice = serde_json::from_value(finalized_response).map_err(|e| {
            StripeServiceError::Configuration(format!(
                "Failed to parse finalized invoice response: {}",
                e
            ))
        })?;
        ...
        if invoice.status.as_ref().map(|s| s.as_str()) == Some("open") {
            ...
            match pay_response {
                Ok(paid_response) => {
                    invoice = serde_json::from_value(paid_response).map_err(|e| {
                        StripeServiceError::Configuration(format!(
                            "Failed to parse paid invoice response: {}",
                            e
                        ))
                    })?;
        ...
        info!(
            "Successfully created and finalized invoice: {} with {} items for customer: {}",
            invoice.id,
            if fee_amount_cents > 0 { "2" } else { "1" },
            customer_id
        );
        Ok(invoice)
    }
    ...
    pub async fn cleanup_pending_invoice_items(
        &self,
        customer_id: &str,
    ) -> Result<(), StripeServiceError> {
        info!(
            "Cleaning up pending invoice items for customer: {}",
            customer_id
        );
        ...
        let list_response = self
            .make_stripe_request_with_idempotency(
                reqwest::Method::GET,
                &format!(
                    "invoiceitems?customer={}&pending=true&limit=100",
                    customer_id
                ),
                None,
                None,
        ...
        info!(
            "Found {} pending invoice items for customer: {}",
            items.len(),
            customer_id
        );
        ...
        info!(
            "Completed cleanup of pending invoice items for customer: {}",
            customer_id
        );
        Ok(())
    }
    ...
    pub async fn cleanup_pending_invoice_items_except_currency(
        &self,
        customer_id: &str,
        keep_currency: &str,
    ) -> Result<usize, StripeServiceError> {
        info!(
            "Cleaning up pending invoice items not in {} for customer: {}",
            keep_currency, customer_id
        );
            ...
            if let Some(item_id) = item.get("id").and_then(|id| id.as_str()) {
                ...
                if currency.to_lowercase() != keep_currency.to_lowercase() {
                        ...
                        .await
                    {
                        Ok(_) => {
                            info!(
                                "Deleted mismatched currency invoice item: {} (was {} {}, keeping only {})",
                        ...
                        Err(e) => {
                            warn!(
                                "Failed to delete invoice item {} (amount: {} {}): {}",
                    ...
                    debug!(
                        "Keeping invoice item {} in matching currency {}",
                        item_id, currency
                    );
                }
            }
        ...
        if total_deleted > 0 {
            info!(
                "Deleted {} invoice items with non-{} currencies for customer: {}",
    ...
    /// Returns the number of items deleted
    pub async fn cleanup_all_pending_invoice_items(
        &self,
        customer_id: &str,
    ) -> Result<usize, StripeServiceError> {
        info!(
            "Cleaning up ALL pending invoice items (all currencies) for customer: {}",
            customer_id
        );
        ...
        while has_more {
            ...
            info!(
                "Found {} pending invoice items in this batch for customer: {}",
                items.len(),
                customer_id
            );
            ...
            for item in &items {
                if let Some(item_id) = item.get("id").and_then(|id| id.as_str()) {
                        ...
                        .await
                    {
                        Ok(_) => {
                            info!(
                                "Deleted pending invoice item: {} (amount: {} {})",
                        ...
                        Err(e) => {
                            ...
                            warn!(
                                "Failed to delete pending invoice item {} (amount: {} {}): {}",
                                item_id, amount, currency, e
                            );
                        }
                    }
        ...
        if total_deleted > 0 {
            info!(
                "Successfully deleted {} pending invoice items for customer: {}",
                total_deleted, customer_id
            );
        }
