...
use crate::db::repositories::api_usage_repository::{DetailedUsageRecord, DetailedUsageResponse, UsageSummary};
...
impl ModelProviderCache {
    ...
    async fn refresh(&self, pool: &Pool<Postgres>) -> Result<(), AppError> {
        ...
        self.models.clear();
        for row in rows {
            self.models.insert(row.id.clone(), ModelInfo {
                name: row.name,
...
impl UsageProcessingService {
    ...
    async fn get_raw_usage_data(
        ...
        Ok(rows.into_iter().map(|row| RawUsageRecord {
            id: row.id,
    ...
    fn process_usage_data(&self, raw_records: Vec<RawUsageRecord>) -> Vec<DetailedUsageRecord> {
        ...
        for record in resolved_records {
            let entry = aggregates.entry(record.model_id.clone()).or_insert_with(|| {
