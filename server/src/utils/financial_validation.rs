...
pub fn validate_financial_amount(
    ...
    if amount.fractional_digit_count() > MAX_DECIMAL_PLACES as i64 {
        return Err(AppError::InvalidArgument(format!(
            "{} amount cannot exceed {} decimal places: {}",
    ...
    if amount != &normalize_cost(amount) {
        return Err(AppError::InvalidArgument(format!(
            "{} amount would lose precision during normalization: {}",
            operation_name, amount
        )));
    }
...
pub fn validate_balance_adjustment(
    ...
    if potential_new_balance < BigDecimal::from(0) {
        return Err(AppError::InvalidArgument(format!(
            "{} would result in negative balance. Current: {}, Adjustment: {}, Resulting: {}",
...
pub fn validate_credit_purchase_amount(amount: &BigDecimal) -> Result<(), AppError> {
    ...
    if amount != &normalize_cost(amount) {
        return Err(AppError::InvalidArgument(format!(
            "Credit purchase amount would lose precision during normalization: {}",
            amount
        )));
    }
...
pub fn validate_credit_refund_amount(amount: &BigDecimal) -> Result<(), AppError> {
    ...
    if amount != &normalized_amount {
        return Err(AppError::InvalidArgument(format!(
            "Credit refund amount has precision errors. Original: {}, Normalized: {}",
...
pub fn validate_credit_adjustment_amount(amount: &BigDecimal) -> Result<(), AppError> {
    ...
    if amount != &normalized_amount {
        return Err(AppError::InvalidArgument(format!(
            "Credit adjustment amount has precision errors. Original: {}, Normalized: {}",
            amount, normalized_amount
        )));
    }
    ...
    if amount.fractional_digit_count() > MAX_DECIMAL_PLACES as i64 {
        return Err(AppError::InvalidArgument(format!(
            "Credit adjustment amount cannot exceed {} decimal places: {}",
