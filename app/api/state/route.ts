...
// Create a rate limiter config specific to the state API
const rateLimiterConfig = createDefaultRateLimiterConfig();
// Customize for this endpoint's needs
rateLimiterConfig.standardCooldown = 15000; // 15 second cooldown
rateLimiterConfig.criticalKeyCooldown = 30000; // 30 second cooldown for critical keys
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error(`[API state GET] Error fetching state for key ${key}:`, error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error(`[API state POST] Error saving state for key ${key}:`, error);
    return NextResponse.json(
