...
export default function ProjectDirectorySelector({
  ...
  useEffect(() => {
    ...
    if (!userEditedRef.current) {
      ...
      const normalizedContextDir = normalizePathForComparison(contextProjectDir);
  ...
  useEffect(() => {
    if (!projectIsLoading && projectDirectory) {
      ...
      const normalizedInput = normalizePathForComparison(inputValue);
      const normalizedProjectDir = normalizePathForComparison(projectDirectory);

      if (normalizedInput === normalizedProjectDir && userEditedRef.current) {
        console.log(`[ProjectDirSelector] Resetting userEditedRef: input is in sync with project directory`);
  ...
  const validateDirectory = useCallback(async (path: string): Promise<{ isValid: boolean; validatedPath?: string; message?: string }> => {
    // Handle empty input
    if (!path.trim()) {
      setValidationStatus({
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : "Failed to validate directory";
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    if (normalizePathForComparison(normalizedInput) === normalizePathForComparison(normalizedProjectDir)) {
  ...
  }, [inputValue, projectDirectory, validateDirectory, setProjectDirectory, showNotification]);
      ...
      <form onSubmit={handleSubmit} className="w-full">
        <div className="flex flex-col space-y-2 w-full">
          ...
          <div className="flex items-center gap-2 w-full">
            <div className="relative flex-1">
              <Input
                ...
                aria-describedby={validationStatus ? "validation-message" : undefined}
              />
              {inputValue && (
                <button
                  type="button"
          ...
          <div className="text-xs text-muted-foreground mt-1 px-1 text-balance">
            Sets the root directory for file browsing, session storage, and project settings.
          </div>
        </div>
      </form>
