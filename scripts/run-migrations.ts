...
async function main() {
  try {
    ...
    console.log("2. Run 'pnpm check-db:repair' to repair database structure (preserves data)");
    console.log("3. Run 'pnpm reset-db' as a last resort (WARNING: deletes all data!)");
    process.exit(1);
  } finally {
    closeDatabase();
  }
