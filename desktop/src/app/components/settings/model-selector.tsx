...
export function ModelSelector({ providers, selectedModelId, onSelect }: ModelSelectorProps) {
  const [activeProviderId, setActiveProviderId] = useState(() => {
    const selectedModel = providers.flatMap(p => p.models).find(m => m.id === selectedModelId);
    return selectedModel?.provider || providers[0]?.provider?.code || "";
  });
              ...
              <div className="space-y-1">
                {providers.map((provider) => (
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    onClick={() => setActiveProviderId(provider.provider.code)}
                    className={cn(
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="space-y-1 text-xs">
                            ...
                            <div className="flex justify-between">
                              ...
                              <span>{formatPrice(model.pricePerInputToken)}/token</span>
                            </div>
                            <div className="flex justify-between">
                              <span>Output Price:</span>
                              <span>{formatPrice(model.pricePerOutputToken)}/token</span>
                            </div>
                          </div>
                        </div>
                      </TooltipContent>
