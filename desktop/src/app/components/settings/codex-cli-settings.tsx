...
// Codex CLI routing is disabled - default value is false (no routing)
// const CODEX_CLI_SETTING_KEY = "codex_cli_enabled";

export default function CodexCliSettings() {
  const { showNotification } = useNotification();
  // Codex CLI routing disabled - keeping default as false
  ...
  useEffect(() => {
    const loadDatabasePath = async () => {
      try {
        setIsLoading(true);
        const path = await invoke<string>("get_database_path_command");
        ...
        await logError(err, "CodexCliSettings.loadDatabasePath");

        showNotification({
          title: "Error Loading Database Path",
          message: userMessage,
          type: "error",
        });
      } finally {
  ...
  // Codex CLI routing toggle handler - disabled
