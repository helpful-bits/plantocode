...
async function getTableInfo(table: string): Promise<
  Array<{
    name: string;
    type: string;
    notnull: number;
...
async function diagnoseProblem(
  ...
  console.log(
    `  - Has 'file_path' column: ${hasFilePathColumn ? "Yes ✅" : "No ❌"}`
  );
  ...
  try {
    await connectionPool.withConnection((db: Database.Database) => {
      try {
        const rows = db
          .prepare(`SELECT id, session_id, path FROM ${table} LIMIT 5`)
          .all();
        console.log(
          `  - 'SELECT path' query test: Success ✅ - Found ${rows.length} rows`
        );
      } catch (err: unknown) {
        const errorMessage = err instanceof Error ? err.message : String(err);
        console.log(
          `  - 'SELECT path' query test: Failed ❌ - ${errorMessage}`
        );
      }
    }, true);
  } catch (error: unknown) {
      ...
      await connectionPool.withConnection((db: Database.Database) => {
        try {
          ...
          if (row) {
            ...
            console.log(
              `  - Actual columns in first row: ${Object.keys(row).join(", ")}`
            );
          } else {
