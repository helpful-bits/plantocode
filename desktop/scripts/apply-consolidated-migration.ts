...
async function main() {
  try {
    ...
    if (!fs.existsSync(migrationFile)) {
      console.error(
        `❌ ERROR: Consolidated migration file not found at ${migrationFile}`
      );
      process.exit(1);
    }
    ...
    await connectionPool.withTransaction(async (db) => {
      for (const statement of statements) {
        try {
          ...
          console.warn(
            `Warning: Statement failed: ${err instanceof Error ? err.message : String(err)}`
          );
    ...
    console.error(
      "❌ ERROR applying consolidated migration:",
