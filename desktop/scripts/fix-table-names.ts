...
async function fixTableNames() {
  ...
  try {
    ...
    await connectionPool.withTransaction(async (db) => {
      ...
      if (
        tables.includes("project_settings_new") &&
        !tables.includes("project_settings")
      ) {
        ...
        console.log(
          "Project_settings table is already correct or does not exist."
        );
      }
    });
