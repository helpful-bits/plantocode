...
/**
 * This script migrates the database from the old location (.o1-pro-flow) to the new location (.vibe-manager)
 *
 * Features:
...
const NEW_APP_DATA_DIR = path.join(os.homedir(), ".vibe-manager");
const NEW_DB_FILE = path.join(NEW_APP_DATA_DIR, "vibe-manager.db");

async function migrateDatabase() {
  console.log("=== Database Migration Utility ===");
  console.log(
    `This tool will migrate your database from ${OLD_DB_FILE} to ${NEW_DB_FILE}`
  );
  console.log("");
  ...
  if (fs.existsSync(NEW_DB_FILE)) {
    console.log(
      "New database already exists. Checking if migration is still needed..."
    );
    ...
    if (oldStats.mtimeMs > newStats.mtimeMs || oldStats.size > newStats.size) {
      console.log(
        "Old database is newer or larger than existing new database."
      );
      ...
      console.log(
        "Existing new database appears up-to-date. No migration needed."
      );
      return;
    }
  }
  ...
  try {
    ...
    const shouldDelete = await promptUser(
      "Would you like to delete the old database? (yes/no): "
    );
    if (shouldDelete.toLowerCase() === "yes") {
      ...
      console.log(
        "Old database will not be deleted. You can manually delete it later if needed."
      );
    }

    console.log("");
    console.log(
      "Migration process complete. Your application will now use the new database location."
    );
  } catch (error) {
    console.error("Error during database migration:", error);
    console.error(
      "Migration failed. Please try again or contact support for assistance."
    );
    process.exit(1);
  }
}
