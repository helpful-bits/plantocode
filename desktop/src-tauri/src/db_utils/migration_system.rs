...
impl MigrationSystem {
    ...
    pub async fn run_migrations(
        ...
        if stored_version.is_none() {
            info!("No stored version found. Setting current version: {}", current_version);
        ...
        let stored = match Version::parse(&stored_version_str) {
            Ok(v) => v,
            Err(e) => {
                warn!("Failed to parse stored version '{}': {}", stored_version_str, e);
    ...
    fn get_applicable_migrations(
        ...
        for migration in &applicable {
            info!("  - {} (priority: {}): {}", 
                migration.id, 
                migration.priority,
                migration.description.as_ref().unwrap_or(&"No description".to_string())
            );
        }
    ...
    fn version_matches(&self, pattern: &str, version: &Version) -> bool {
        ...
        match pattern {
            ...
            pattern if pattern.contains("..") => {
                ...
                if parts.len() == 2 {
                    if let (Ok(start), Ok(end)) = (Version::parse(parts[0]), Version::parse(parts[1])) {
    ...
    async fn execute_migration(
        ...
        // Load migration SQL
        let migration_sql = self.load_migration_file(app_handle, &migration.migration_file).await?;
