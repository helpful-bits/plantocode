...
pub fn ensure_db_permissions(app_data_root_dir: &Path) -> AppResult<()> {
    ...
    #[cfg(unix)] {
        ...
        info!("Setting permissions for directory: {}", db_dir_path.display());
    ...
    #[cfg(windows)] {
        ...
        info!("Database directory exists (Windows): {}", db_dir_path.display());
    }
    ...
    if db_file_path.exists() {
        #[cfg(unix)] {
            ...
            info!("Setting permissions for database file: {}", db_file_path.display());
            ...
            if wal_file_path.exists() {
                info!("Setting permissions for WAL file: {}", wal_file_path.display());
                fs::set_permissions(&wal_file_path, file_perms.clone())?;
            }
            
            if shm_file_path.exists() {
                info!("Setting permissions for SHM file: {}", shm_file_path.display());
...
pub async fn handle_readonly_database(app_handle: &tauri::AppHandle, app_data_root_dir: &Path) -> AppResult<bool> {
    ...
    if !db_file_path.exists() {
        warn!("Database file doesn't exist, nothing to recover: {}", db_file_path.display());
        return Ok(false);
    }
    ...
    info!("Creating database backup: {} -> {}", db_file_path.display(), backup_path.display());
    ...
    match sqlx::query("PRAGMA integrity_check;").fetch_one(&db).await {
        Ok(row) => {
            match row.try_get::<'_, String, _>(0) {
                Ok(result) => {
                    if result == "ok" {
                    ...
                    match row.try_get::<'_, String, _>(0) {
                        Ok(result) => {
                            if result == "ok" {
                                ...
                                warn!("Database integrity check failed after removing auxiliary files: {}", result);
                            }
                        },
                        Err(e) => {
                            warn!("Failed to read integrity check result after removing auxiliary files: {}", e);
                        }
                    }
                },
                Err(e) => {
                    error!("Failed to open database after removing auxiliary files: {}", e);
                    ...
                    warn!("Database completely reset - application needs to restart and reinitialize database");
