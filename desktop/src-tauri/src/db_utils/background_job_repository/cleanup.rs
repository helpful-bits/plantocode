...
impl BackgroundJobRepository {
    ...
    pub async fn clear_job_history(&self, days_to_keep: i64) -> AppResult<()> {
        ...
        if days_to_keep == -2 {
            ...
            // Delete all completed, failed, or canceled jobs (excluding implementation plans and merged plans)
            sqlx::query(
                "DELETE FROM background_jobs WHERE status IN ($1, $2, $3) AND task_type NOT IN ($4, $5)",
            )
            .bind(JobStatus::Completed.to_string())
            .bind(JobStatus::Failed.to_string())
            .bind(JobStatus::Canceled.to_string())
            .bind(crate::models::TaskType::ImplementationPlan.to_string())
            .bind(crate::models::TaskType::ImplementationPlanMerge.to_string())
            .execute(&*self.pool)
            .await
            .map_err(|e| AppError::DatabaseError(format!("Failed to delete job history: {}", e)))?;
        } else if days_to_keep > 0 {
            // Delete jobs older than specified days (excluding implementation plans and merged plans)
            let target_date_ts = current_ts - (days_to_keep * 24 * 60 * 60 * 1000); // Convert days to milliseconds

            sqlx::query("DELETE FROM background_jobs WHERE status IN ($1, $2, $3) AND created_at < $4 AND task_type NOT IN ($5, $6)")
                .bind(JobStatus::Completed.to_string())
                .bind(JobStatus::Failed.to_string())
                .bind(JobStatus::Canceled.to_string())
                .bind(target_date_ts)
            ...
            // Delete jobs older than 90 days (default cleanup, excluding implementation plans and merged plans)
    ...
    pub async fn clear_all_completed_jobs(&self) -> AppResult<usize> {
        // Delete completed, failed, or canceled jobs (excluding implementation plans and merged plans)
    ...
    pub async fn clear_completed_jobs_for_session(&self, session_id: &str) -> AppResult<usize> {
        // Delete completed, failed, or canceled jobs for the session (excluding implementation plans and merged plans)
        let result = sqlx::query("DELETE FROM background_jobs WHERE session_id = $1 AND status IN ($2, $3, $4) AND task_type NOT IN ($5, $6)")
            .bind(session_id)
            .bind(JobStatus::Completed.to_string())
            .bind(JobStatus::Failed.to_string())
            .bind(JobStatus::Canceled.to_string())
