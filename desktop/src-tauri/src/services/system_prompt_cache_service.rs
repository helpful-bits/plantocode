...
impl SystemPromptCacheService {
    ...
    pub async fn get_fresh_system_prompt(
        &self,
        task_type: &str,
        ...
        // Check cache for given task_type
        {
            ...
            if let Some(entry) = cache.get(task_type) {
                ...
                if age_seconds <= CACHE_TTL_SECONDS {
                    debug!(
                        "Retrieved system prompt for task type '{}' from in-memory cache",
                        task_type
                    );
        ...
        match self
            .server_client
            .get_default_system_prompt(task_type)
            .await
        {
            Ok(prompt) => {
                ...
                debug!(
                    "Cached system prompt for task type '{}' (found: {})",
                    task_type,
                    prompt.is_some()
                );
                Ok(prompt)
            ...
            Err(e) => {
                warn!(
                    "Failed to fetch system prompt for task type '{}': {}",
                    task_type, e
                );
                Err(e)
            }
