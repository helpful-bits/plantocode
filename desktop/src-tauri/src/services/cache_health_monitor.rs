...
impl CacheHealthMonitor {
            ...
            match self.cache_health_check().await {
                Ok(metrics) => {
                    if !metrics.is_healthy {
                        warn!("Cache health check failed. Health score: {:.2}, Age: {}s", 
                        ...
                        info!("Cache health check passed. Health score: {:.2}, Size: {} entries", 
    ...
    pub async fn cache_health_check(&self) -> Result<CacheHealthMetrics, AppError> {
        ...
        let (cache_size, cache_entries) = match cache.lock() {
            ...
            Err(e) => {
                ...
                return Err(AppError::ConfigError(format!("Failed to access cache: {}", e)));
            }
        };
        ...
        let health_score = self.calculate_health_score(cache_size, cache_age_seconds, stale_entries_count);
    ...
    async fn invalidate_stale_cache_entries(&self) -> Result<(), AppError> {
        ...
        match cache.lock() {
            Ok(mut cache_guard) => {
                ...
                let keys_to_remove: Vec<String> = cache_guard.iter()
            ...
            Err(e) => {
                ...
                Err(AppError::ConfigError(format!("Failed to invalidate cache: {}", e)))
            }
        }
    }
    ...
    async fn update_last_refresh_timestamp(&self) -> Result<(), AppError> {
        ...
        match cache.lock() {
            ...
            Err(e) => {
                ...
                Err(AppError::ConfigError(format!("Failed to update cache metadata: {}", e)))
            }
        }
    }
    ...
    fn get_last_refresh_timestamp(&self, cache_entries: &HashMap<String, JsonValue>) -> u64 {
        cache_entries.get("_cache_metadata_last_refresh")
            .and_then(|v| v.as_u64())
            .unwrap_or(0)
    }
    ...
    #[instrument(skip(self))]
    fn calculate_health_score(&self, cache_size: usize, cache_age_seconds: u64, stale_entries_count: usize) -> f64 {
        let mut score = 1.0;
...
#[instrument(skip(app_handle))]
pub async fn get_cache_health_metrics(app_handle: &AppHandle) -> Result<CacheHealthMetrics, AppError> {
    let monitor = CacheHealthMonitor::new(app_handle.clone());
