...
pub async fn list_directories_only(
    directory_path: &str,
) -> AppResult<Vec<crate::models::DirectoryInfo>> {
    ...
    let system_directory_paths = [
        ...
        "/Library/Devices",
        "/Library/Trial",
        "/Library/NamespaceDescriptors",
        "/Library/factorPackSets",
        "/.Spotlight-V100",
    ...
    if !fs_utils::is_directory(&directory)
        .await
    ...
    let entries = fs_utils::list_directory(&directory)
        .await
    ...
    for entry in entries {
        if entry.is_dir {
            ...
            let is_root_level = dir_str == "/"
                || dir_str.starts_with("/Users/") && dir_str.matches('/').count() == 2;
