...
pub fn map_server_proxy_error(status_code: u16, response_text: &str) -> AppError {
    ...
    if let Ok(error_response) = serde_json::from_str::<ServerErrorResponse>(response_text) {
        ...
        match error_type.as_str() {
            ...
            "plan_upgrade_required" | "upgrade_required" => AppError::PlanUpgradeRequired(message),
            ...
            "subscription_expired" => AppError::SubscriptionExpired(message),
            "subscription_cancelled" => AppError::SubscriptionCancelled(message),
            ...
            "subscription_conflict" => AppError::SubscriptionConflict(message),
            "spending_limit_exceeded" => AppError::SpendingLimitExceeded(message),
            "invoice_error" => AppError::InvoiceError(message),
            "checkout_error" | "checkout_session_expired" | "checkout_cancelled" => AppError::CheckoutError(message),
