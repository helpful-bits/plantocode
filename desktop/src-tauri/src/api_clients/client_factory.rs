...
pub fn get_api_client(app_handle: &AppHandle) -> AppResult<Arc<dyn ApiClient>> {
    app_handle
        .try_state::<Arc<dyn ApiClient>>()
        .ok_or_else(|| {
            AppError::InternalError("ApiClient not managed in application state".to_string())
        })
        .map(|s| s.inner().clone())
}
...
pub fn get_server_proxy_client(app_handle: &AppHandle) -> AppResult<Arc<ServerProxyClient>> {
    app_handle
        .try_state::<Arc<ServerProxyClient>>()
        .ok_or_else(|| {
            AppError::InternalError(
                "ServerProxyClient not managed in application state".to_string(),
            )
        })
        .map(|s| s.inner().clone())
}
...
pub fn get_transcription_client(app_handle: &AppHandle) -> AppResult<Arc<dyn TranscriptionClient>> {
    app_handle
        .try_state::<Arc<dyn TranscriptionClient>>()
        .ok_or_else(|| {
            AppError::InternalError(
                "TranscriptionClient not managed in application state".to_string(),
            )
        })
        .map(|s| s.inner().clone())
}
