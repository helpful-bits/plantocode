...
impl StreamedResponseHandler {
    ...
    pub async fn process_stream<S>(&self, mut stream: S) -> AppResult<StreamResult>
    where
        S: futures::Stream<Item = Result<StreamEvent, crate::error::AppError>> + Unpin,
    {
        ...
        while let Some(event_result) = stream.next().await {
            match event_result {
                Ok(event) => {
                    match event {
                        StreamEvent::StreamStarted { request_id } => {
                            ...
                            if let Err(e) =
                                crate::db_utils::job_metadata_updates::update_job_request_id(
                                    &self.repo,
                                    &self.job_id,
                                    &request_id,
                                )
                            ...
                            {
                                warn!(
                                    "Failed to update job {} with request_id {}: {}",
                                    self.job_id, request_id, e
                                );
                            }
                        }
                        StreamEvent::ContentChunk(chunk) => {
                            // Check if job has been canceled before processing chunk
                            if job_processor_utils::check_job_canceled(&self.repo, &self.job_id)
                                .await?
                            {
                            ...
                            if let Some(chunk_usage) = chunk.usage {
                                if current_usage.is_none() {
                                    ...
                                    current_usage = Some(OpenRouterUsage {
                                        ...
                                        prompt_tokens_details: chunk_usage
                                            .prompt_tokens_details
                                            .map(|details| OpenRouterPromptTokensDetails {
                            ...
                            if last_update.elapsed() >= update_interval {
                                ...
                                {
                                    let tokens_output =
                                        current_usage.as_ref().unwrap().completion_tokens as f32;
                                    let progress =
                                        (tokens_output / self.config.max_tokens as f32) * 100.0;
                        ...
                        StreamEvent::UsageUpdate(usage_update) => {
                            ...
                            info!(
                                "Processing authoritative usage update for job {}: input={}, output={}, total={}, cost={}",
                                self.job_id,
                            ...
                            current_usage = Some(OpenRouterUsage {
                                ...
                                cache_write_tokens: usage_update.cache_write_tokens.unwrap_or(0)
                                    as i32,
                                cache_read_tokens: usage_update.cache_read_tokens.unwrap_or(0)
                                    as i32,
                            ...
                            {
                                error!(
                                    "Failed to update job stream usage with authoritative data: {}",
                                    e
                                );
                            }
                        }
                        StreamEvent::StreamCancelled {
                            ...
                            info!(
                                "Stream cancelled for job {}: request_id={}, reason={}",
                        ...
                        StreamEvent::ErrorDetails {
                            ...
                            error!(
                                "Detailed error received for job {}: request_id={}, error={:?}",
                            ...
                            if let Err(e) = self
                                .repo
                                .update_job_error_details(&self.job_id, &error)
                                .await
                            {
                        ...
                        StreamEvent::StreamCompleted {
                            ...
                            info!(
                                "Stream completed successfully for job {} with final cost: ${:.6} (input={}, output={}, cache_read={}, cache_write={})",
                                self.job_id,
                                final_cost,
                                tokens_input,
                                tokens_output,
                            ...
                            debug!(
                                "Received final stream data: cost={}, tokens={:?}/{:?}",
                ...
                Err(e) => {
                    ...
                    warn!(
                        "Streaming error occurred for job {} - authoritative usage will be preserved if available: {:?}",
        ...
        info!(
            "Performing final database update for job {} with 100% progress",
            self.job_id
        );
        if let Err(e) = self
            .repo
        ...
        let usage_result = if let Some(usage) = current_usage {
            if let Some(cost) = usage.cost {
                info!(
                    "Stream processing completed for job {} with authoritative cost: ${:.6} (tokens: input={}, output={}, total={})",
                    self.job_id,
                    cost,
                    usage.prompt_tokens,
                ...
                debug!(
                    "Stream processing completed for job {} with usage data but no cost field (tokens: input={}, output={}, total={})",
            ...
            debug!(
                "Stream processing completed for job {} - no usage data received during stream, server will provide usage data through other channels",
                self.job_id
            );
    ...
    pub async fn process_stream_from_client_with_messages(
        ...
        debug!(
            "Starting streaming call with structured messages for job {}",
            self.job_id
        );

        // Execute streaming call with structured messages
        let stream = llm_client
            .chat_completion_stream(messages, api_options)
            .await?;
