...
pub(super) fn load_workflow_definitions_from_files()
-> AppResult<HashMap<String, Arc<WorkflowDefinition>>> {
    let mut workflow_definitions = HashMap::new();

    // Get the path to the workflow_definitions directory
    let workflow_definitions_dir =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("src/jobs/workflow_definitions");
    ...
    let entries = fs::read_dir(&workflow_definitions_dir).map_err(|e| {
    ...
    for entry in entries {
        let entry = entry
            .map_err(|e| AppError::JobError(format!("Failed to read directory entry: {}", e)))?;
        let path = entry.path();
        ...
        if path.extension().map(|ext| ext == "json").unwrap_or(false) {
            ...
            let content = fs::read_to_string(&path).map_err(|e| {
                AppError::JobError(format!("Failed to read workflow file {:?}: {}", path, e))
            })?;

            let workflow_definition: WorkflowDefinition =
                serde_json::from_str(&content).map_err(|e| {
                    AppError::JobError(format!("Failed to parse workflow file {:?}: {}", path, e))
                })?;
    ...
    if workflow_definitions.is_empty() {
        return Err(AppError::JobError(
            "No valid workflow definitions found".to_string(),
        ));
    }
