...
pub(super) async fn retry_workflow_stage_internal(
    ...
    workflow_definitions: &tokio::sync::Mutex<
        std::collections::HashMap<String, std::sync::Arc<WorkflowDefinition>>,
    >,
...
pub(super) async fn retry_workflow_stage_with_config_internal(
    ...
    info!(
        "Retrying workflow stage {:?} for workflow {}, original job {}",
    ...
    reset_subsequent_stages_internal(
        workflows,
        &workflow_state,
        &stage_to_retry,
        &workflow_definition,
    ...
    let stage_payload = super::payload_builder::create_abstract_stage_payload(
        app_handle,
        &workflow_state,
        stage_definition,
        &workflow_definition,
    ...
    let model_settings = super::stage_scheduler::get_stage_model_config_internal(
        &stage_to_retry,
    ...
    // Update the workflow state to replace the failed job with the new one
    {
        ...
        if let Some(workflow) = workflows.get_mut(workflow_id) {
            ...
            info!(
                "Added retry job {} for stage {:?} in workflow {}",
                new_job_id, stage_to_retry, workflow_id
            );
        }
    }
...
pub(super) async fn reset_subsequent_stages_internal(
    ...
    // Find the stage definition for the retry stage
    let retry_stage_def = workflow_definition
        .stages
        .iter()
    ...
    let subsequent_stage_names =
        find_subsequent_stages_in_definition(workflow_definition, &retry_stage_def.stage_name);

    // Convert stage names back to task types for job matching
    let subsequent_task_types: Vec<TaskType> = subsequent_stage_names
        .iter()
    ...
    if let Some(workflow) = workflows_guard.get_mut(&workflow_state.workflow_id) {
        for task_type in subsequent_task_types {
            ...
            if let Some(workflow_stage) = task_type_to_workflow_stage(task_type) {
                if let Some(stage_job) = workflow
                    .stages
                    .iter_mut()
                    .find(|job| job.name == workflow_stage.to_string())
                {
                    if matches!(stage_job.status, JobStatus::Queued | JobStatus::Running) {
                        stage_job.status = JobStatus::Canceled;
                        stage_job.error_message =
                            Some("Cancelled due to retry of earlier stage".to_string());
                        info!(
                            "Cancelled stage job {} for stage {:?} due to retry",
                            stage_job.job_id, workflow_stage
                        );
                    }
                }
            }
