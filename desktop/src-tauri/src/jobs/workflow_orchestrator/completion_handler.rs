...
pub(super) async fn handle_stage_completion_internal(
    ...
    store_stage_data_fn: impl Fn(
        &str,
        serde_json::Value,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = crate::error::AppResult<()>> + Send>,
    >,
) -> AppResult<()> {
    ...
    {
        warn!(
            "Stage data extraction failed for job {}: {} - failing the stage",
            job_id, e
        );

        // Explicitly fail the stage
        orchestrator
            .handle_stage_failure(&workflow_id, job_id, Some(e.to_string()))
            .await?;
        return Err(AppError::JobError(format!(
            "Stage data extraction failed for stage {}: {}",
            job_id, e
        )));
    }

    debug!(
        "Successfully extracted and stored stage data from job {}",
        job_id
    );

    // Log current workflow state for debugging
    debug!(
        "Checking workflow completion for workflow {} after stage completion",
        workflow_id
    );
    ...
    let workflow_state_for_payload_building = {
        ...
        workflows_guard
            ...
            .ok_or_else(|| {
                AppError::JobError(format!(
                    "Workflow not found after data extraction: {}",
                    workflow_id
                ))
            })?
            .clone()
    ...
    let task_type = workflow_state_for_payload_building
        .stages
        .iter()
    ...
    if task_type == TaskType::RegexFileFilter {
        ...
        if workflow_state_for_payload_building
            .intermediate_data
            ...
            .is_empty()
        {
            ...
            if let Some(workflow_state) = workflows_guard.get_mut(workflow_id) {
                workflow_state.intermediate_data.workflow_completion_message =
    ...
    if task_type == TaskType::FileRelevanceAssessment {
        let token_count = workflow_state_for_payload_building
            .intermediate_data
            .ai_filtered_files_token_count;
        if token_count.unwrap_or(0) >= 120_000 {
            info!(
                "Extended path finding stages are being skipped due to large file context size ({} tokens)",
    ...
    let next_stages = super::stage_scheduler::find_next_abstract_stages_to_execute_internal(
        &workflow_state_for_payload_building,
        &workflow_definition,
    )
    .await;

    debug!(
        "Found {} next stages to execute for workflow {}",
        next_stages.len(),
        workflow_id
    );
    for stage in &next_stages {
        debug!(
            "Next stage eligible: {} (task_type: {:?})",
            stage.stage_name, stage.task_type
        );
    }
    ...
    if next_stages.is_empty() {
        ...
        if workflow_state_for_payload_building.should_stop() {
            if workflow_state_for_payload_building.has_cancelled() {
                orchestrator
                    .mark_workflow_failed(workflow_id, "Workflow stopped due to user cancellation")
                    .await?;
                info!("Workflow {} stopped due to cancellation", workflow_id);
            } else if workflow_state_for_payload_building.has_failed() {
                orchestrator
                    .mark_workflow_failed(workflow_id, "Workflow stopped due to stage failure")
                    .await?;
            ...
            let is_complete = super::workflow_utils::is_workflow_complete(
                &workflow_state_for_payload_building,
                &workflow_definition,
            );
            ...
            if is_complete {
                ...
                debug!(
                    "Workflow {} not complete - checking individual stage status",
                    workflow_id
                );
                for stage_job in &workflow_state_for_payload_building.stages {
                    debug!(
                        "Stage {:?} ({}): status={:?}",
                        stage_job.task_type, stage_job.job_id, stage_job.status
                    );
                }
        ...
        let currently_running =
            super::stage_scheduler::count_running_jobs_in_workflow_internal(workflows, workflow_id)
                .await;
        ...
        for stage_def in stages_to_start {
            info!(
                "Starting next stage: {} for workflow: {}",
                stage_def.stage_name, workflow_id
            );
            if let Err(e) = orchestrator
                .create_abstract_stage_job(
                    &workflow_state_for_payload_building,
                    stage_def,
                    &workflow_definition,
                )
            ...
            {
                error!(
                    "Failed to create next stage job for {}: {}",
