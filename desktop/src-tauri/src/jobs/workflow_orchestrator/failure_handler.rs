    ...
    match workflow_error_handler.handle_stage_failure(
                ...
                match response.should_continue {
                    ...
                    false => {
                        ...
                        if workflow.status != WorkflowStatus::Failed {
                            ...
                            drop(workflows_guard); // Release lock before async call
                            super::event_emitter::emit_workflow_status_event_internal(app_handle, &workflow_state, &format!("Workflow failed: {}", failure_reason)).await;
                            return Ok(());
                        }
                    }
                }
        ...
        Err(e) => {
            ...
            if let Some(workflow) = workflows_guard.get_mut(workflow_id) {
                ...
                drop(workflows_guard); // Release lock before async call
                super::event_emitter::emit_workflow_status_event_internal(app_handle, &workflow_clone, &format!("Workflow failed: {}", failure_reason)).await;
            } else {
                // Fallback to mark workflow failed directly
                super::state_updater::mark_workflow_failed_internal(workflows, workflow_id, &format!("Error handler failed: {}", e)).await?;
            }
        }
    }
