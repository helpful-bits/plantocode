...
use crate::jobs::workflow_types::{
    WorkflowDefinition, WorkflowStage, WorkflowStageDefinition, WorkflowStageJob, WorkflowState,
    WorkflowStatus,
};
...
pub async fn start_workflow_internal(
    ...
    info!(
        "Starting workflow '{}': {}",
        workflow_definition_name, workflow_id
    );
    ...
    if entry_stages.is_empty() {
        return Err(AppError::JobError(format!(
            "No entry stages found in workflow definition: {}",
            workflow_definition_name
        )));
    }

    debug!(
        "Found {} entry stages for workflow '{}': {:?}",
        entry_stages.len(),
        workflow_definition_name,
        entry_stages
            .iter()
    ...
    // Store the workflow state and create entry stage jobs in the same lock scope
    {
        ...
        for entry_stage in entry_stages {
            debug!(
                "Creating entry stage '{}' for workflow '{}'",
                entry_stage.stage_name, workflow_definition_name
            );
            create_abstract_stage_job_with_lock(
                &mut workflows_guard,
                app_handle,
                &workflow_state,
                entry_stage,
...
pub async fn cancel_workflow_with_reason_internal(
    ...
    info!(
        "Canceled {} jobs for workflow {}, {} failures",
    ...
    let background_job_repo =
        match app_handle.try_state::<Arc<crate::db_utils::BackgroundJobRepository>>() {
    ...
    event_emitter::emit_workflow_status_event_internal(
        app_handle,
        &workflow_state,
        &format!("Workflow canceled: {}", reason),
    )
    .await;
...
pub async fn resume_workflow_internal(
...

    {
        ...
        if let Some(workflow_state) = workflows_guard.get_mut(workflow_id) {
            ...
            emit_workflow_status_event_internal(app_handle, workflow_state, "Workflow resumed")
                .await;
    ...
    if let Err(e) = start_next_abstract_stages_internal(
        workflows,
        app_handle,
    ...
    {
        warn!(
            "Failed to start next stages after resuming workflow {}: {}",
            workflow_id, e
        );
    }
...
async fn create_abstract_stage_job_with_lock(
    ...
    let model_settings = get_stage_model_config_for_definition(
        app_handle,
        stage_definition,
    ...
    if let Some(workflow) = workflows.get_mut(&workflow_state.workflow_id) {
        let depends_on = if stage_definition.dependencies.is_empty() {
            ...
            debug!(
                "Stage '{}' depends on: {:?}",
                stage_definition.stage_name, stage_definition.dependencies
            );
        ...
        debug!(
            "Adding stage job '{}' to workflow '{}' with dependency: {:?}",
        ...
        if workflow.stages.iter().any(|job| job.job_id == job_id) {
            debug!(
                "Successfully registered job {} in workflow {} stage_jobs",
                job_id, workflow_state.workflow_id
            );
        } else {
            error!(
                "Failed to register job {} in workflow {} stage_jobs",
                job_id, workflow_state.workflow_id
            );
        }
    } else {
        error!(
            "Workflow {} not found when trying to add stage job {}",
            workflow_state.workflow_id, job_id
        );
    }
...
async fn start_next_abstract_stages_internal(
    ...
    let workflow_definition = {
        ...
        definitions
            ...
            .ok_or_else(|| {
                AppError::JobError(format!(
                    "Workflow definition not found: {}",
                    workflow_state.workflow_definition_name
                ))
            })?
    };
    ...
    if next_stages.is_empty() {
        ...
        let currently_running =
            stage_scheduler::count_running_jobs_in_workflow_internal(workflows, workflow_id).await;
...
async fn get_stage_model_config_for_definition(
    ...
    let settings_repo = app_handle
        .state::<Arc<crate::db_utils::settings_repository::SettingsRepository>>()
        .inner()
        .clone();
...
async fn emit_workflow_status_event_internal(
    app_handle: &AppHandle,
    workflow_state: &WorkflowState,
    message: &str,
) {
