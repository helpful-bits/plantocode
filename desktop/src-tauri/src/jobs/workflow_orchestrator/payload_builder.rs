    ...
    match task_type {
        ...
        TaskType::FileRelevanceAssessment => {
            ...
            let payload = FileRelevanceAssessmentPayload {
                task_description: workflow_state.task_description.clone(),
                locally_filtered_files: workflow_state.intermediate_data.locally_filtered_files.clone(),
            };
        ...
        TaskType::WebSearchExecution => {
            ...
            if prompts.is_empty() {
                ...
                debug!("Using {} web search prompts for WebSearchExecution payload", prompts.len());
            }
