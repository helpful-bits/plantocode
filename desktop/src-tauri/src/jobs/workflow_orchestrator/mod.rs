...
use super::workflow_types::{
    AbstractWorkflowState, WorkflowDefinition, WorkflowIntermediateData, WorkflowResult,
    WorkflowStage, WorkflowStageDefinition, WorkflowStageEvent, WorkflowStageJob, WorkflowState,
...
impl WorkflowOrchestrator {
    ...
    async fn get_workflow_definitions(
        &self,
    ) -> Result<MutexGuard<HashMap<String, Arc<WorkflowDefinition>>>, WorkflowError> {
        ...
        {
            Ok(guard) => Ok(guard),
            Err(_) => Err(WorkflowError::LockTimeout(format!(
                "Failed to acquire workflow definitions lock within {}ms",
                LOCK_TIMEOUT_MS
            ))),
        }
    }
    ...
    fn safe_job_status_from_str(s: &str) -> Result<JobStatus, WorkflowError> {
        JobStatus::from_str(s).map_err(|e| {
            WorkflowError::JobStatusParseError(format!("Invalid job status '{}': {}", s, e))
        })
    }
    ...
    fn safe_task_type_from_str(s: &str) -> Result<TaskType, WorkflowError> {
        TaskType::from_str(s).map_err(|e| {
            WorkflowError::TaskTypeParseError(format!("Invalid task type '{}': {}", s, e))
        })
    }
    ...
    fn safe_duration_as_millis(duration: Duration) -> Result<u64, WorkflowError> {
        duration.as_millis().try_into().map_err(|_| {
            WorkflowError::DurationError(format!(
                "Duration overflow: {} ms exceeds u64 maximum",
                duration.as_millis()
            ))
        })
    }
        ...
        match self.get_workflow_definitions().await {
            ...
            Err(e) => {
                return Err(AppError::JobError(format!(
                    "Failed to store workflow definitions: {}",
                    e
                )));
            }
        }
    ...
    pub async fn cancel_workflow_with_reason(
        &self,
        workflow_id: &str,
        reason: &str,
    ) -> AppResult<()> {
    ...
    pub async fn get_workflow_status(&self, workflow_id: &str) -> AppResult<WorkflowState> {
        ...
        if let Err(e) = self.recover_workflow_if_needed(workflow_id).await {
            error!(
                "Failed to recover workflow {} if needed: {}",
                workflow_id, e
            );
        }
    ...
    pub async fn update_job_status(
        ...
        state_updater::update_job_status_internal(
            &self.workflows,
            job_id,
            status.clone(),
            error_message.clone(),
        ...
        if let Some(stage_job) =
            query_service::get_stage_job_by_id_internal(&self.workflows, job_id).await?
        {
            event_emitter::emit_workflow_stage_event_internal(
                &self.app_handle,
                &workflow_id,
                &stage_job,
                &status,
        ...
        match status {
            JobStatus::Completed => {
                if let Err(e) = self
                    .handle_stage_completion(&workflow_id, job_id, job_result_data)
                    .await
                {
            ...
            JobStatus::Failed => {
                if let Err(e) = self
                    .handle_stage_failure(&workflow_id, job_id, error_message)
                    .await
                {
    ...
    pub async fn add_stage_job_to_workflow(
        ...
        if let Some(workflow_state) = workflows_guard.get_mut(workflow_id) {
            ...
            Err(crate::error::AppError::JobError(format!(
                "Workflow {} not found",
                workflow_id
            )))
        }
    ...
    pub async fn recover_orphaned_jobs(&self) -> AppResult<()> {
        ...
        let active_jobs = background_job_repo
            .get_jobs_by_status(&[
                    ...
                    if let Some(workflow_id) = metadata.get("workflowId").and_then(|v| v.as_str()) {
                        ...
                        if let Some(workflow_state) = workflows_guard.get_mut(workflow_id) {
                            ...
                            if !workflow_state
                                .stages
                                .iter()
                                .any(|stage_job| stage_job.job_id == job.id)
                            {
                                // This is an orphaned job - add it to the workflow
                                if let Ok(task_type) =
                                    crate::models::TaskType::from_str(&job.task_type)
                                {
                                    let stage_name = match task_type {
                                        crate::models::TaskType::RegexFileFilter => {
                                            "RegexFileFilter"
                                        }
                                        crate::models::TaskType::FileRelevanceAssessment => {
                                    ...
                                    recovered_count += 1;
                                    info!(
                                        "Recovered orphaned job {} for workflow {}",
                                        job.id, workflow_id
                                    );
                                }
                            }
        ...
        if recovered_count > 0 {
            ...
            let workflows_to_check: Vec<(String, Vec<String>)> = {
                ...
                workflows_guard
                    .iter()
                    .filter(|(_, workflow_state)| {
                        workflow_state.status
                            == crate::jobs::workflow_types::WorkflowStatus::Running
                    })
                    .map(|(workflow_id, workflow_state)| {
                        let completed_jobs: Vec<String> = workflow_state
                            .stages
                            .iter()
                            .filter(|stage_job| {
            ...
            for (workflow_id, completed_job_ids) in workflows_to_check.iter() {
                for job_id in completed_job_ids.iter() {
                    if let Err(e) = self
                        .handle_stage_completion(&workflow_id, &job_id, None)
                        .await
                    {
                        error!(
                            "Failed to handle stage completion during recovery for job {}: {}",
                            job_id, e
                        );
                    }
                }
    ...
    async fn recover_workflow_if_needed(&self, workflow_id: &str) -> AppResult<()> {
        ...
        let workflow_jobs = background_job_repo
            ...
            for job in workflow_jobs {
                    ...
                    .any(|stage_job| stage_job.job_id == job.id)
                {
                    ...
                    if let Ok(task_type) = crate::models::TaskType::from_str(&job.task_type) {
                        ...
                        recovered_count += 1;
                        info!(
                            "Lazy recovery: added job {} to workflow {}",
                            job.id, workflow_id
                        );
                    }
                }
        ...
        if recovered_count > 0 {
            info!(
                "Lazy recovered {} jobs for workflow {}",
                recovered_count, workflow_id
            );
            ...
            if let Some(workflow_state) = workflows_guard.get(workflow_id) {
                ...
                for job_id in completed_jobs.iter() {
                    ...
                    {
                        error!(
                            "Failed to handle stage completion during lazy recovery for job {}: {}",
                            job_id, e
                        );
                    }
                }
    ...
    pub async fn get_workflow_state_by_id(
        &self,
        workflow_id: &str,
        ...
        workflows.retain(|_, workflow| {
            match workflow.status {
                WorkflowStatus::Completed | WorkflowStatus::Failed | WorkflowStatus::Canceled => {
                    workflow
                        .completed_at
                        .map_or(true, |completed| completed > cutoff_time)
                }
    ...
    async fn handle_stage_completion(
        &self,
        workflow_id: &str,
        job_id: &str,
        job_result_data: Option<crate::jobs::types::JobResultData>,
    ...
    async fn handle_stage_cancellation(&self, workflow_id: &str, job_id: &str) -> AppResult<()> {
        ...
        let workflow_state = {
            ...
            if let Some(workflow) = workflows.get_mut(workflow_id) {
                if !matches!(
                    workflow.status,
                    crate::jobs::workflow_types::WorkflowStatus::Completed
                ...
                return Err(AppError::JobError(format!(
                    "Workflow {} not found for cancellation",
                    workflow_id
                )));
            }
        };
    ...
    pub async fn create_abstract_stage_job(
        ...
        let mut workflows = self.workflows.lock().await;
        self.create_abstract_stage_job_with_lock(
            &mut workflows,
            workflow_state,
            stage_definition,
            workflow_definition,
    ...
    pub async fn start_next_abstract_stages(&self, workflow_id: &str) -> AppResult<()> {
        ...
        if workflow_state.status == crate::jobs::workflow_types::WorkflowStatus::Paused {
            debug!(
                "Workflow {} is paused, not starting new stages",
                workflow_id
            );
            return Ok(());
        }
        ...
        let workflow_definitions_guard = self.get_workflow_definitions().await.map_err(|e| {
            AppError::JobError(format!("Failed to get workflow definitions: {}", e))
        })?;

        let workflow_definition = workflow_definitions_guard
            .get(&workflow_state.workflow_definition_name)
            .cloned() // Clone the Arc<WorkflowDefinition>
            .ok_or_else(|| {
                AppError::JobError(format!(
                    "Workflow definition '{}' not found for workflow {}",
                    workflow_state.workflow_definition_name, workflow_id
                ))
            })?;

        // Find all stages that can be executed in parallel using abstract definitions
        let next_stages = stage_scheduler::find_next_abstract_stages_to_execute_internal(
            &workflow_state,
            &workflow_definition,
        )
        .await;
        ...
        if next_stages.is_empty() {
            ...
            if workflow_utils::is_workflow_complete(&workflow_state, &workflow_definition) {
                ...
                self.mark_workflow_failed(workflow_id, "One or more stages failed")
                    .await?;
            } else {
            ...
            if available_slots == 0 {
                debug!(
                    "Cannot start more stages for workflow {} - concurrency limit reached ({} running)",
                    workflow_id, currently_running
                );
                return Ok(());
            }
            ...
            for stage_def in stages_to_start {
                info!(
                    "Starting abstract stage: {} for workflow: {}",
                    stage_def.stage_name, workflow_id
                );
                if let Err(e) = self
                    .create_abstract_stage_job(&workflow_state, stage_def, &workflow_definition)
                    .await
                {
                    error!(
                        "Failed to create abstract stage job for {}: {}",
                        stage_def.stage_name, e
                    );
    ...
    fn get_dependency_job_for_data_extraction<'a>(
        &self,
        workflow_state: &'a WorkflowState,
        dependency_task_type: TaskType,
    ) -> AppResult<&'a super::workflow_types::WorkflowStageJob> {
    ...
    fn get_latest_job_for_stage<'a>(
        &self,
        workflow_state: &'a WorkflowState,
        dependency_task_type: TaskType,
    ) -> Option<&'a super::workflow_types::WorkflowStageJob> {
    ...
    pub async fn mark_workflow_completed(&self, workflow_id: &str) -> AppResult<()> {
        let workflow_state =
            state_updater::mark_workflow_completed_internal(&self.workflows, workflow_id).await?;
        ...
        let workflow_result =
            super::workflow_types::WorkflowResult::from_workflow_state(&workflow_state);

        // Serialize workflow result
        let result_json = serde_json::to_string(&workflow_result).map_err(|e| {
            AppError::JobError(format!("Failed to serialize workflow result: {}", e))
        })?;
        ...
        match self
            .workflow_cleanup_handler
            .cleanup_workflow(workflow_id, &self.app_handle)
            .await
        {
            Ok(cleanup_result) => {
                info!(
                    "Cleanup completed for workflow: {} - cleaned {} jobs",
                    workflow_id,
                    cleanup_result.cleaned_jobs.len()
                );
            }
            Err(e) => {
                warn!(
                    "Failed to cleanup resources for completed workflow {}: {}",
                    workflow_id, e
                );
            }
        }
    ...
    pub async fn mark_workflow_failed(
        ...
        let workflow_state = state_updater::mark_workflow_failed_internal(
            &self.workflows,
            workflow_id,
            error_message,
        )
            ...
            .await
        {
            Ok(cleanup_result) => {
                info!(
                    "Cleanup completed for failed workflow: {} - cleaned {} jobs",
                    workflow_id,
                    cleanup_result.cleaned_jobs.len()
                );
            }
            Err(e) => {
                warn!(
                    "Failed to cleanup resources for failed workflow {}: {}",
                    workflow_id, e
                );
            }
        }
    ...
    async fn create_stage_payload_for_retry(
        &self,
        ...
        let workflow_definitions = self.get_workflow_definitions().await.map_err(|e| {
            AppError::JobError(format!("Failed to get workflow definitions: {}", e))
        })?;
        ...
        let workflow_definition =
            workflow_definitions
                .get("FileFinderWorkflow")
                .ok_or_else(|| {
                    AppError::JobError("FileFinderWorkflow definition not found".to_string())
                })?;

        // Find the stage definition for this task type
        let stage_definition = workflow_definition
            .stages
            .iter()
            .find(|stage_def| stage_def.task_type == task_type)
            .ok_or_else(|| {
                AppError::JobError(format!(
                    "Stage definition not found for task type: {:?}",
                    task_type
                ))
            })?;

        // Use abstract payload creation
        payload_builder::create_abstract_stage_payload(
            &self.app_handle,
            workflow_state,
            stage_definition,
            workflow_definition,
    ...
    pub async fn reset_subsequent_stages(
        &self,
        workflow_state: &WorkflowState,
        ...
        retry_handler::reset_subsequent_stages_internal(
            &self.workflows,
            workflow_state,
            retry_stage,
            &workflow_definition,
...
pub async fn init_workflow_orchestrator(
    app_handle: AppHandle,
) -> AppResult<Arc<WorkflowOrchestrator>> {
    ...
    if let Err(_) = WORKFLOW_ORCHESTRATOR.set(orchestrator.clone()) {
        return Err(AppError::JobError(
            "Failed to initialize workflow orchestrator".to_string(),
        ));
    }
