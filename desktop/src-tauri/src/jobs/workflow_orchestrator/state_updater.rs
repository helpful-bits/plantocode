...
use crate::jobs::workflow_types::{WorkflowStage, WorkflowState, WorkflowStatus};
    ...
    match stage {
        ...
        WorkflowStage::RegexFileFilter => {
            ...
            if let Some(files_array) = stage_data.as_array() {
                workflow_state.intermediate_data.locally_filtered_files = files_array
                    .iter()
                    .filter_map(|v| v.as_str().map(String::from))
                    .collect();
                debug!(
                    "Stored {} locally filtered files from RegexFileFilter in intermediate_data",
                    workflow_state
                        .intermediate_data
        ...
        WorkflowStage::FileRelevanceAssessment => {
            if let Some(files) = stage_data.get("relevantFiles").and_then(|v| v.as_array()) {
                workflow_state.intermediate_data.ai_filtered_files = files
                    .iter()
                    .filter_map(|v| v.as_str().map(String::from))
                    .collect();
                debug!(
                    "Stored {} AI filtered files in intermediate_data",
                    workflow_state.intermediate_data.ai_filtered_files.len()
                );
            } else {
                warn!(
                    "FileRelevanceAssessment stage_data missing or invalid 'relevantFiles' field, keeping existing data"
                );
            }

            workflow_state
                .intermediate_data
                .ai_filtered_files_token_count = stage_data
                .get("tokenCount")
                .and_then(|v| v.as_u64())
        ...
        WorkflowStage::WebSearchPromptsGeneration => {
            ...
            if let Some(prompts_array) = stage_data.get("prompts").and_then(|v| v.as_array()) {
                workflow_state.intermediate_data.web_search_prompts = prompts_array
                    .iter()
                    .filter_map(|v| v.as_str().map(String::from))
                    .collect();
                debug!(
                    "Stored {} sophisticated research prompts in intermediate_data",
        ...
        WorkflowStage::WebSearchExecution => {
            ...
            if let Some(results_array) = stage_data.get("searchResults").and_then(|v| v.as_array())
            {
                ...
                workflow_state.intermediate_data.web_search_results = results_array
                    .iter()
                    .map(|v| serde_json::to_string(v).unwrap_or_else(|_| v.to_string()))
                    .collect();
                debug!(
                    "Stored {} sophisticated research results in intermediate_data",
...
pub(super) async fn mark_workflow_failed_internal(
    ...
    if let Some(workflow_state) = workflows_guard.get_mut(workflow_id) {
        ...
        workflow_state.updated_at = workflow_state
            .completed_at
...
pub(super) async fn update_job_status_internal(
    ...
    for (workflow_id, workflow_state) in workflows_guard.iter_mut() {
            ...
            .find(|job| job.job_id == job_id)
        {
            ...
            workflow_state.total_actual_cost = Some(
                workflow_state
                    .stages
                    .iter()
                    .filter_map(|j| j.actual_cost)
                    .sum(),
            );
            ...
            debug!(
                "Updated job {} status to {:?} in workflow {}",
            ...
            if let Some(workflow_stage) = WorkflowStage::from_task_type(&stage_job.task_type) {
                match update_intermediate_data_internal(workflow_state, &workflow_stage, stage_data)
                {
                    Ok(_) => {
                        ...
                        debug!(
                            "Stored stage data for job {} in workflow {}",
                            job_id, workflow_id
                        );
                        return Ok(());
                    }
...
pub(super) async fn atomic_stage_completion_update(
    ...
    for (workflow_id, workflow_state) in workflows_guard.iter_mut() {
        ...
        let stage_job_info = workflow_state
            .stages
            .iter()
            .find(|job| job.job_id == job_id)
            .map(|job| {
        ...
        )) = stage_job_info
        {
            ...
            if let Some(data) = stage_data {
                if let Some(workflow_stage) = WorkflowStage::from_task_type(&task_type) {
                    match update_intermediate_data_internal(workflow_state, &workflow_stage, data) {
                        Ok(_) => {
                            ...
                            workflow_state.updated_at = current_time;
                            debug!(
                                "Atomically updated job {} status to {:?} and stored stage data in workflow {}",
                                job_id, status, workflow_id
                            );
                ...
                workflow_state.updated_at = current_time;
                debug!(
                    "Atomically updated job {} status to {:?} in workflow {}",
                    job_id, status, workflow_id
                );
                return Ok(workflow_id.clone());
            }
