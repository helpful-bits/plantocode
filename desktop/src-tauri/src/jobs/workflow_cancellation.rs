...
mod tests {
    ...
    #[tokio::test]
    async fn test_subsequent_stages_calculation() {
        let repo = Arc::new(BackgroundJobRepository::new("test.db".to_string()));
        let handler = WorkflowCancellationHandler::new(repo);
    ...
    #[tokio::test]
    async fn test_coordinator_race_condition_protection() {
        let repo = Arc::new(BackgroundJobRepository::new("test.db".to_string()));
        let coordinator = CancellationCoordinator::new(repo);
        
        assert!(!coordinator.is_cancellation_in_progress("workflow_123"));
        
        // In a real test, we would spawn concurrent cancellation attempts
        // and verify only one proceeds
    }
}