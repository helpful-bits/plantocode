...
use crate::jobs::types::{Job, JobPayload, JobUIMetadata};
...
pub fn convert_db_job_to_job(db_job: &BackgroundJob) -> AppResult<Job> {
    ...
    let task_type = TaskType::from_str(&db_job.task_type).map_err(|e| {
        AppError::JobError(format!(
            "Failed to parse task type '{}': {}",
            db_job.task_type, e
        ))
    })?;
...
pub fn deserialize_value_to_job_payload(
    ...
    match task_type {
        TaskType::RegexFileFilter => {
            let workflow_payload: RegexFileFilterPayload =
                serde_json::from_value(json_value.clone()).map_err(|e| {
                    AppError::JobError(format!(
                        "Failed to deserialize RegexFileFilterPayload: {}",
                        e
                    ))
                })?;
        ...
        TaskType::ExtendedPathFinder => {
            let payload: ExtendedPathFinderPayload = serde_json::from_value(json_value.clone())
                .map_err(|e| {
                    AppError::JobError(format!(
                        "Failed to deserialize ExtendedPathFinderPayload: {}",
                        e
                    ))
                })?;
        ...
        TaskType::ImplementationPlan => {
            let payload: ImplementationPlanPayload = serde_json::from_value(json_value.clone())
                .map_err(|e| {
                    AppError::JobError(format!(
                        "Failed to deserialize ImplementationPlanPayload: {}",
                        e
                    ))
                })?;
        ...
        TaskType::TaskRefinement => {
            let payload: TaskRefinementPayload = serde_json::from_value(json_value.clone())
                .map_err(|e| {
                    AppError::JobError(format!(
                        "Failed to deserialize TaskRefinementPayload: {}",
                        e
                    ))
                })?;
        ...
        TaskType::TextImprovement => {
            let payload: TextImprovementPayload = serde_json::from_value(json_value.clone())
                .map_err(|e| {
                    AppError::JobError(format!(
                        "Failed to deserialize TextImprovementPayload: {}",
                        e
                    ))
                })?;
        ...
        TaskType::GenericLlmStream => {
            let payload: GenericLlmStreamPayload = serde_json::from_value(json_value.clone())
                .map_err(|e| {
                    AppError::JobError(format!(
                        "Failed to deserialize GenericLlmStreamPayload: {}",
                        e
                    ))
                })?;
        ...
        TaskType::FileRelevanceAssessment => {
            let payload: FileRelevanceAssessmentPayload =
                serde_json::from_value(json_value.clone()).map_err(|e| {
                    AppError::JobError(format!(
                        "Failed to deserialize FileRelevanceAssessmentPayload: {}",
                        e
                    ))
                })?;
        ...
        TaskType::WebSearchPromptsGeneration => {
            let payload: WebSearchPromptsGenerationPayload =
                serde_json::from_value(json_value.clone()).map_err(|e| {
                    AppError::JobError(format!(
                        "Failed to deserialize WebSearchPromptsGenerationPayload: {}",
                        e
                    ))
                })?;
        ...
        TaskType::WebSearchExecution => {
            let payload: WebSearchExecutionPayload = serde_json::from_value(json_value.clone())
                .map_err(|e| {
                    AppError::JobError(format!(
                        "Failed to deserialize WebSearchExecutionPayload: {}",
                        e
                    ))
                })?;
        ...
        TaskType::FileFinderWorkflow => {
            let payload: FileFinderWorkflowPayload = serde_json::from_value(json_value.clone())
                .map_err(|e| {
                    AppError::JobError(format!(
                        "Failed to deserialize FileFinderWorkflowPayload: {}",
                        e
                    ))
                })?;
        ...
        TaskType::WebSearchWorkflow => {
            let payload: WebSearchWorkflowPayload = serde_json::from_value(json_value.clone())
                .map_err(|e| {
                    AppError::JobError(format!(
                        "Failed to deserialize WebSearchWorkflowPayload: {}",
                        e
                    ))
                })?;
        ...
        TaskType::VoiceTranscription => Err(AppError::JobError(
            "VoiceTranscription uses direct API calls instead of background job system".to_string(),
        )),
        // ImplementationPlanMerge task type
        TaskType::ImplementationPlanMerge => {
            let payload: ImplementationPlanMergePayload =
                serde_json::from_value(json_value.clone()).map_err(|e| {
                    AppError::JobError(format!(
                        "Failed to deserialize ImplementationPlanMergePayload: {}",
                        e
                    ))
                })?;
        ...
        TaskType::Streaming | TaskType::Unknown => Err(AppError::JobError(format!(
            "Task type {:?} is not supported for JSON to JobPayload conversion",
            task_type
        ))),
    }
}
