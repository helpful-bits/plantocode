...
impl JobProcessor for RootFolderSelectionProcessor {
    ...
    async fn process(&self, job: Job, app_handle: AppHandle) -> AppResult<JobProcessResult> {
        ...
        let llm_config = job_processor_utils::get_llm_task_config(&db_job, &app_handle, &session).await?;
        let (model_used, temperature, max_output_tokens) = llm_config;

        let task_config_builder = LlmTaskConfigBuilder::new(model_used, temperature, max_output_tokens);
        let task_runner = LlmTaskRunner::new(app_handle.clone(), job.clone(), task_config_builder.build());
        ...
        info!("Providing {} directory paths to LLM for selection (max depth: 2)", sorted_paths.len());
        ...
        let llm_response = task_runner.execute_llm_task(prompt_context, &_settings_repo).await?;

        let parsed_paths: Vec<String> = if let Ok(json_array) = serde_json::from_str::<Vec<String>>(&llm_response.response) {
            json_array
        } else {
        ...
        let actual_cost = llm_response.usage
            .as_ref()
            .and_then(|u| u.cost)
            .unwrap_or(0.0);

        let job_result = JobProcessResult::success(
            job.id.clone(),
            JobResultData::Json(result_json)
        )
        .with_tokens(
            llm_response.usage.as_ref().map(|u| u.prompt_tokens as u32),
            llm_response.usage.as_ref().map(|u| u.completion_tokens as u32),
        )
        .with_system_prompt_template(llm_response.system_prompt_template.clone())
        .with_actual_cost(actual_cost);

        repo.update_job_status(&db_job.id, &JobStatus::Completed, None).await?;
