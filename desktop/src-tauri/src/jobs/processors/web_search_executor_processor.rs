    ...
    async fn process(&self, job: Job, app_handle: AppHandle) -> AppResult<JobProcessResult> {
        ...
        let prompts = match &job.payload {
            JobPayload::WebSearchExecution(payload) => {
                payload.prompts.clone()
            }
            _ => {
                return Err(AppError::JobError(
                    "Invalid payload type for WebSearchExecutor - expected WebSearchExecution".to_string(),
                ));
            }
        };
        ...
        let prompt_tasks = prompts_to_execute.into_iter().enumerate().map(|(index, prompt)| {
            ...
            async move {
                ...
                let title = extract_task_title(&prompt).unwrap_or_else(|| format!("Research Task {}", index + 1));
                ...
                task_runner.execute_llm_task(prompt_context, &settings_repo).await
        ...
        if successful_results.is_empty() {
            let error_msg = format!("All sophisticated research prompts failed. Failures: {}", failed_prompts.join("; "));
        ...
        debug!("Sophisticated research execution completed for job {}", job.id);
