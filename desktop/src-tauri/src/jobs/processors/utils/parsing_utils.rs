...
const COMMON_WORDS: &[&str] = &[
    "the", "is", "are", "was", "were", "been", "have", "has", "had", "do", "does", "did", "will",
    "would", "could", "should", "may", "might", "must", "can", "this", "that", "these", "those",
    "a", "an", "and", "or", "but", "if", "then", "because", "as", "while", "when", "where",
    "which", "who", "what", "how", "why", "all", "some", "any", "each", "every",
];
...
lib/utils.js"#;
        ...
        assert_eq!(
            paths,
            vec!["src/main.rs", "lib/utils.js", "config/settings.json"]
        );
    }
}
...
pub fn parse_implementation_plan(
    clean_xml_content: &str,
) -> AppResult<(StructuredImplementationPlan, String)> {
    ...
    if clean_xml_content.trim().is_empty() {
        ...
        return Err(AppError::ValidationError(
            "Empty XML content provided".to_string(),
        ));
    }
    ...
    if !is_xml_format {
        warn!(
            ...
            &clean_xml_content[..100.min(clean_xml_content.len())]
        );
    ...
    match quick_xml::de::from_str::<StructuredImplementationPlan>(clean_xml_content) {
        ...
        Err(e) => {
            warn!(
                "Failed to parse structured XML: {}. Content length: {}",
                e,
                clean_xml_content.len()
            );
...
pub fn create_fallback_plan_from_text(
    text_content: &str,
) -> AppResult<(StructuredImplementationPlan, String)> {
    ...
    let fallback_plan = StructuredImplementationPlan {
        agent_instructions: Some(
            "Note: This plan was parsed from text format. The LLM did not return XML as expected."
                .to_string(),
        ),
        steps,
    };
    ...
    // Generate summary
    let summary = format!(
        "Implementation Plan with {} steps (parsed from text format)",
        fallback_plan.steps.len()
    );
