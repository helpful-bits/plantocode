...
use crate::models::{RuntimeAIConfig, TaskType, TaskSpecificModelConfig};
...
impl ConfigValidator {
    pub fn new() -> Self {
        ...
        validator.register_rule("task_type_configurations", Box::new(TaskTypeConfigurationRule));
    ...
    pub fn validate_complete_configuration(&self, config: &RuntimeAIConfig, app_handle: &AppHandle) -> ValidationResult {
        let mut all_errors = Vec::new();
        let mut all_warnings = Vec::new();
        let mut critical_failures = 0;
        
        info!("Starting comprehensive configuration validation with {} rules", self.validation_rules.len());
        ...
        let summary = if is_valid {
            format!("Configuration validation passed with {} warnings", all_warnings.len())
        } else {
            format!("Configuration validation FAILED with {} critical errors, {} total errors, {} warnings", 
                   critical_failures, all_errors.len(), all_warnings.len())
        };
    ...
    pub fn fail_fast_on_invalid_config(&self, config: &RuntimeAIConfig, app_handle: &AppHandle) -> AppResult<()> {
        let result = self.validate_complete_configuration(config, app_handle);
        
        if !result.is_valid {
            let critical_errors: Vec<&ValidationError> = result.errors.iter()
                .filter(|e| matches!(e.severity, ValidationSeverity::Critical))
                .collect();
            
            if !critical_errors.is_empty() {
                let error_messages: Vec<String> = critical_errors.iter()
        ...
        for warning in &result.warnings {
            warn!("Configuration warning in {}: {}", warning.component, warning.message);
        }
                ...
                Some(task_config) => {
                    ...
                    if task_config.temperature < 0.0 || task_config.temperature > 2.0 {
                        errors.push(ValidationError {
                            ...
                            message: format!("Temperature {} is out of range [0.0, 2.0]", task_config.temperature),
        ...
        for task_key in config.tasks.keys() {
            if !known_task_keys.contains(task_key) {
                warnings.push(ValidationWarning {
                    ...
                    component: format!("TaskConfig[{}]", task_key),
                    message: "Configuration exists on server but no corresponding task type found".to_string(),
                });
            }
        }
        
        let is_valid = errors.iter().all(|e| !matches!(e.severity, ValidationSeverity::Critical));
        let summary = format!("Task type configuration validation: {} errors, {} warnings", errors.len(), warnings.len());
...
impl ValidationRule for ModelAvailabilityRule {
    ...
    fn validate(&self, config: &RuntimeAIConfig, _app_handle: &AppHandle) -> ValidationResult {
        ...
        let available_models: std::collections::HashSet<String> = config.providers
            .iter()
        ...
        for (task_key, task_config) in &config.tasks {
            if !available_models.contains(&task_config.model) {
                errors.push(ValidationError {
                    ...
                    component: format!("TaskConfig[{}]", task_key),
                    field: Some("model".to_string()),
                    message: format!("Model '{}' not found in available providers", task_config.model),
                    severity: ValidationSeverity::Critical,
                });
            }
        }
        ...
        let is_valid = errors.iter().all(|e| !matches!(e.severity, ValidationSeverity::Critical));
        let summary = format!("Model availability validation: {} models available, {} errors", total_models, errors.len());
...
impl ValidationRule for ProviderConsistencyRule {
    ...
    fn validate(&self, config: &RuntimeAIConfig, _app_handle: &AppHandle) -> ValidationResult {
        ...
        let summary = format!("Provider consistency validation: {} providers checked", config.providers.len());
...
impl ValidationRule for ParameterRangeRule {
    ...
    fn validate(&self, config: &RuntimeAIConfig, _app_handle: &AppHandle) -> ValidationResult {
        ...
        let is_valid = errors.iter().all(|e| matches!(e.severity, ValidationSeverity::Low));
        let summary = format!("Parameter range validation: {} configurations checked", config.tasks.len());
...
pub async fn validate_model_availability(app_handle: &AppHandle) -> AppResult<()> {
    ...
    info!("Model availability validation passed: {} models available", total_models);
    Ok(())
}