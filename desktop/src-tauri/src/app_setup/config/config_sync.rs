...
impl ConfigSyncManager {
    ...
    pub async fn is_config_stale(&self) -> AppResult<bool> {
        ...
        if is_stale {
            debug!("Configuration is stale. Last sync: {}, Current: {}, Interval: {}s", 
    ...
    pub async fn validate_config_integrity(&self) -> AppResult<bool> {
        ...
        let available_model_ids: std::collections::HashSet<String> = runtime_config.providers
            .iter()
        ...
        debug!("Configuration integrity check passed. {} models available", total_models);
        Ok(true)
    }
    ...
    pub async fn force_sync(&self) -> AppResult<()> {
        ...
        let current_time = get_timestamp();
        self.settings_repo.set_value("config_last_sync_timestamp", &current_time.to_string()).await?;
...
pub async fn initialize_config_sync(app_handle: &AppHandle) -> AppResult<()> {
    ...
    let sync_manager_bg = ConfigSyncManager::new(app_handle.clone(), 
                                                app_handle.state::<Arc<SettingsRepository>>().inner().clone());
