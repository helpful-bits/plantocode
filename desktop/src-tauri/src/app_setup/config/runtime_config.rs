...
pub async fn fetch_and_update_runtime_ai_config(app_handle: &AppHandle) -> Result<(), AppError> {
    ...
    if total_models == 0 {
        ...
        return Err(AppError::ConfigError("No models available from server".to_string()));
    } else {
...
fn validate_task_type_configurations(runtime_config: &RuntimeAIConfig) -> Result<(), AppError> {
            ...
            match runtime_config.tasks.get(&task_key) {
                Some(task_config) => {
                    ...
                    if task_config.max_tokens == 0 {
                        invalid_configs.push(format!("{}: max_tokens must be positive, got 0", task_key));
                    }
                    if task_config.temperature < 0.0 || task_config.temperature > 2.0 {
                        invalid_configs.push(format!("{}: temperature {} is out of range [0.0, 2.0]", task_key, task_config.temperature));
                    }
    ...
    let unknown_configs: Vec<String> = runtime_config.tasks.keys()
...
pub async fn validate_all_required_configs_present(app_handle: &AppHandle) -> Result<(), AppError> {
    ...
    if !cache_guard.contains_key("runtime_ai_config") {
        return Err(AppError::ConfigError("Runtime AI config not found in cache".to_string()));
    }
    ...
    if runtime_config.tasks.is_empty() {
        return Err(AppError::ConfigError("No task configurations found".to_string()));
    }
...
fn validate_runtime_config_before_cache(runtime_config: &RuntimeAIConfig) -> Result<(), AppError> {
    ...
    for (idx, provider) in runtime_config.providers.iter().enumerate() {
        if provider.models.is_empty() {
            return Err(AppError::ConfigError(format!("Provider {} has no models", idx)));
        }
        ...
        for model in &provider.models {
            if model.id.trim().is_empty() {
                return Err(AppError::ConfigError(format!("Model in provider {} has empty ID", idx)));
            }
        }
    }
    
    info!("Validated {} models across {} providers", total_models, runtime_config.providers.len());
    
    // Validate task configurations with zero tolerance
    for (task_key, task_config) in &runtime_config.tasks {
        if task_config.model.trim().is_empty() {
            return Err(AppError::ConfigError(format!("Task '{}' has empty model", task_key)));
        }
        if task_config.max_tokens == 0 {
            return Err(AppError::ConfigError(format!("Task '{}' has zero max_tokens", task_key)));
        }
        if task_config.temperature < 0.0 || task_config.temperature > 2.0 {
...
pub async fn detect_configuration_drift(app_handle: &AppHandle) -> Result<Vec<String>, AppError> {
    ...
    let fresh_config_value = server_proxy_client.get_runtime_ai_config().await
    ...
    for (task_key, fresh_task_config) in &fresh_config.tasks {
        match cached_config.tasks.get(task_key) {
            ...
            None => {
                drift_issues.push(format!("Task '{}' added on server but not in cache", task_key));
            }
        }
    }
    ...
    for task_key in cached_config.tasks.keys() {
        if !fresh_config.tasks.contains_key(task_key) {
            drift_issues.push(format!("Task '{}' removed from server but still in cache", task_key));
        }
    }
    ...
    if drift_issues.is_empty() {
        ...
        warn!("Configuration drift detected: {} issues found", drift_issues.len());
    }
...
pub async fn purge_invalid_configurations(app_handle: &AppHandle) -> Result<Vec<String>, AppError> {
    ...
    match config_cache.lock() {
        ...
        Err(e) => {
            return Err(AppError::ConfigError(format!("Failed to acquire cache lock: {}", e)));
        }
    }
    ...
    match config_cache.lock() {
        Ok(cache_guard) => {
            if let Some(config_value) = cache_guard.get("runtime_ai_config") {
                ...
                Err(AppError::ConfigError("Runtime AI config not found in cache".to_string()))
            }
        }
        Err(e) => {
            Err(AppError::ConfigError(format!("Failed to acquire cache lock: {}", e)))
        }
    }
}
