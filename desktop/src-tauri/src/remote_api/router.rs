use crate::commands::{
    app_commands, file_system_commands, generic_task_commands, implementation_plan_commands,
    job_commands, session_commands, terminal_commands, text_commands, web_search_commands,
    workflow_commands,
};
...
pub async fn dispatch(
    ...
    match request.method.as_str() {
        ...
        "session.getTaskDescriptionHistory" => {
            handle_session_get_task_description_history(app_handle, request).await
        }
        "session.syncTaskDescriptionHistory" => {
            handle_session_sync_task_description_history(app_handle, request).await
        }
        "session.updateFiles" => handle_session_update_files(app_handle, request).await,
        "session.getFileRelationships" => {
            handle_session_get_file_relationships(app_handle, request).await
        }
        ...
        // Action endpoints
        "actions.readImplementationPlan" => {
            handle_actions_read_implementation_plan(app_handle, request).await
        }
        "actions.findRelevantFiles" => {
            handle_actions_find_relevant_files(app_handle, request).await
        }
        "actions.createImplementationPlan" => {
            handle_actions_create_implementation_plan(app_handle, request).await
        }
        ...
        "actions.refineTaskDescription" => {
            handle_actions_refine_task_description(app_handle, request).await
        }
        "actions.continueWebSearchFromJob" => {
            handle_actions_continue_web_search_from_job(app_handle, request).await
        }
        "actions.retryWorkflowStage" => {
            handle_actions_retry_workflow_stage(app_handle, request).await
        }
        "actions.cancelWorkflowStage" => {
            handle_actions_cancel_workflow_stage(app_handle, request).await
        }

        // Workflow operations
        "workflows.startFileFinder" => {
            handle_workflows_start_file_finder(app_handle, request).await
        }
...
async fn handle_session_list(app_handle: &AppHandle, request: RpcRequest) -> RpcResponse {
    ...
    match session_commands::get_sessions_for_project_command(app_handle.clone(), project_directory)
        .await
    {
...
async fn handle_session_get_task_description_history(
    app_handle: &AppHandle,
    request: RpcRequest,
) -> RpcResponse {
    let session_id = match request.params.get("sessionId") {
    ...
    match session_commands::get_task_description_history_command(app_handle.clone(), session_id)
        .await
    {
...
async fn handle_session_sync_task_description_history(
    app_handle: &AppHandle,
    request: RpcRequest,
) -> RpcResponse {
    let session_id = match request.params.get("sessionId") {
...
async fn handle_session_get_file_relationships(
    app_handle: &AppHandle,
    request: RpcRequest,
) -> RpcResponse {
    let session_id = match request.params.get("sessionId") {
