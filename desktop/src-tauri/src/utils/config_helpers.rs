        ...
        for model_info in &provider.models {
            if model_info.id == model_name {
                ...
                if let Some(context_window) = model_info.context_window {
                    debug!(
                        "Found context window for model {}: {}",
                    ...
                    return Err(AppError::config_error_with_context(
                        ...
                        &format!(
                            "Model '{}' is missing context_window configuration",
                            model_name
                        ),
...
pub async fn get_default_max_tokens_for_task(
    ...
    if !task.requires_llm() {
        return Err(AppError::config_error_with_context(
            "Invalid Task Type",
            &format!(
                "Task '{}' is a local filesystem task that does not require max_tokens configuration",
                task.to_string()
            ),
    ...
    let task_config = runtime_config.tasks.get(&task_key).ok_or_else(|| {
        AppError::missing_task_config_error(&task_key, &["model", "max_tokens", "temperature"])
    })?;
    ...
    debug!(
        "Found max tokens for task {}: {}",
        task_key, task_config.max_tokens
    );
    Ok(task_config.max_tokens)
}

/// Get default temperature for a task (STRICT - NO FALLBACKS)
pub async fn get_default_temperature_for_task(
    task_type: Option<TaskType>,
    app_handle: &AppHandle,
) -> AppResult<f32> {
    ...
    if !task.requires_llm() {
        return Err(AppError::config_error_with_context(
            "Invalid Task Type",
            &format!(
                "Task '{}' is a local filesystem task that does not require temperature configuration",
                task.to_string()
            ),
...
pub async fn get_model_for_task(task_type: TaskType, app_handle: &AppHandle) -> AppResult<String> {
    ...
    if !task_type.requires_llm() {
        return Err(AppError::config_error_with_context(
            "Invalid Task Type",
            &format!(
                "Task '{}' is a local filesystem task that does not require LLM model configuration",
                task_type.to_string()
            ),
...
pub async fn get_max_concurrent_jobs(app_handle: &AppHandle) -> AppResult<usize> {
    ...
    let max_jobs = runtime_config.max_concurrent_jobs.ok_or_else(|| {
        AppError::config_error_with_context(
...
pub async fn get_model_info(
    model_id: &str,
    app_handle: &AppHandle,
) -> AppResult<crate::models::ModelInfo> {
...
pub async fn get_runtime_ai_config_from_cache(
    ...
    let config_value = cache_guard.get("runtime_ai_config").ok_or_else(|| {
        AppError::config_error_with_context(
    ...
    let config = serde_json::from_value::<RuntimeAIConfig>(config_value.clone()).map_err(|e| {
    ...
    for task_type in required_tasks {
        ...
        if task_config.temperature < 0.0 || task_config.temperature > 2.0 {
            return Err(AppError::config_error_with_context(
                ...
                &format!(
                    "Set temperature between 0.0 and 2.0 for task '{}'",
                    task_key
                ),
                None,
            ));
