// Token estimator for desktop-side accurate token counting and estimation
// This module provides local token estimation using tiktoken-rs for:
// 1. Avoiding server requests for token estimation
// 2. Real-time token estimation during prompt composition
// 3. Consistency with server-side token calculations
...
fn get_tokenizer_encoding_for_model(model: &str) -> Result<tiktoken_rs::CoreBPE, AppError> {
    ...
    for (prefix, encoding_name) in PROVIDER_TOKENIZER_MAP.iter() {
        if model_lower.starts_with(prefix) {
            ...
            return match *encoding_name {
                "cl100k_base" => cl100k_base().map_err(|e| AppError::SerdeError(format!("cl100k_base tokenizer error: {}", e))),
                "o200k_base" => o200k_base().map_err(|e| AppError::SerdeError(format!("o200k_base tokenizer error: {}", e))),
                _ => get_bpe_from_model(encoding_name).map_err(|e| AppError::SerdeError(format!("get_bpe_from_model error: {}", e))),
            };
        }
    }
    ...
    cl100k_base().map_err(|e| AppError::SerdeError(format!("cl100k_base fallback tokenizer error: {}", e)))
}
...
/// 
/// This is used for:
/// - Local token estimation (avoiding server requests)
/// - Real-time prompt composition feedback
/// - Consistency with server-side calculations
/// 
/// # Arguments
/// 
