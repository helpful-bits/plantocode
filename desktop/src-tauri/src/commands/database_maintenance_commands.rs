...
pub async fn check_database_health_command(app_handle: AppHandle) -> AppResult<DatabaseHealthData> {
    ...
    let app_data_dir = app_handle
        .path()
        .app_local_data_dir()
        .map_err(|e| crate::error::AppError::DatabaseError(format!("Failed to get app local data directory: {}", e)))?;
    ...
    info!("[DatabaseMaintenance] Database health check completed: {}", health_data.status);
    Ok(health_data)
}
...
pub async fn repair_database_command(app_handle: AppHandle) -> AppResult<serde_json::Value> {
    ...
    if db_path.exists() {
        let backup_file = app_data_dir.join(format!("appdata_backup_{}.db", chrono::Utc::now().timestamp()));
            ...
            .await
            {
                Ok(result) => {
                    if result == "ok" {
                        ...
                        log::info!("[DatabaseMaintenance] Database repair successful - integrity check passed");
...
pub async fn reset_database_command(app_handle: AppHandle) -> AppResult<serde_json::Value> {
    ...
    // Create backup if database exists
    if db_path.exists() {
        let backup_file = app_data_dir.join(format!("appdata_reset_backup_{}.db", chrono::Utc::now().timestamp()));
        
        match fs::copy(&db_path, &backup_file) {
            Ok(_) => {
                backup_path = Some(backup_file.to_string_lossy().to_string());
                log::info!("[DatabaseMaintenance] Created backup before reset at: {:?}", backup_file);
            }
            Err(e) => {
                log::warn!("[DatabaseMaintenance] Failed to create backup before reset: {}", e);
                return Ok(serde_json::json!({
                    "success": false,
    ...
    match app_setup::database::initialize_database(&app_handle).await {
        ...
        Err(e) => {
            log::error!("[DatabaseMaintenance] Failed to re-initialize database after reset: {}", e);
            Ok(serde_json::json!({
                "success": false,
