...
use crate::services::{BackupService, BackupInfo, BackupStats};
...
pub async fn restore_from_backup_command(
    ...
    info!("[BackupCommands] Restoring database from backup: {}", backup_filename);
    ...
    let backup_info = backups
        ...
        .ok_or_else(|| crate::error::AppError::DatabaseError(
            format!("Backup file not found: {}", backup_filename)
        ))?;
    ...
    match backup_service.restore_from_backup(backup_path).await {
        Ok(_) => {
            info!("[BackupCommands] Database restored successfully from: {}", backup_filename);
            Ok(serde_json::json!({
                "success": true,
...
pub async fn auto_restore_latest_backup_command(
    ...
    match backup_service.auto_restore_latest_backup().await? {
        Some(restored_backup) => {
            info!("[BackupCommands] Auto-restored from backup: {}", restored_backup);
            Ok(serde_json::json!({
                "success": true,
...
pub async fn delete_backup_command(
    ...
    match std::fs::remove_file(backup_path) {
        Ok(_) => {
            info!("[BackupCommands] Backup deleted successfully: {}", backup_filename);
            Ok(serde_json::json!({
                "success": true,
                "backup_file": backup_filename,
